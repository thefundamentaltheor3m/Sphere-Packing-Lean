...
\begin{definition}[Smooth cutoff]\label{def:radial-cutoff}\lean{RadialSchwartz.cutoffC}\leanok
  Let $\chi : \R \to \R$ be the smooth transition function with
  \[
    \chi(r) = 0 \text{ for } r \le -\tfrac12, \qquad
    \chi(r) = 1 \text{ for } r \ge 0,
  \]
  (in the formalization, $\chi(r) = \operatorname{smoothTransition}(2r + 1)$), regarded as a
  complex-valued function. It is $C^\infty$ on $\R$.
\end{definition}

\begin{lemma}[Cutoff profiles are globally Schwartz]\label{lemma:cutoff-mul-decay}\uses{def:radial-cutoff}\lean{RadialSchwartz.cutoffC_mul_decay_of_nonneg_of_contDiff, RadialSchwartz.contDiff_cutoffC_mul_of_contDiffOn_Ioi_neg1}\leanok
  Let $f : \R \to \C$.
  \begin{enumerate}
...
\begin{definition}[Schwartz packaging of a profile]\label{def:fCutSchwartz}\uses{def:radial-cutoff, lemma:cutoff-mul-decay, def:Schwartz-Space}\lean{RadialSchwartz.Bridge.fCutSchwartz}\leanok
