...
\end{lemma}
\begin{proof}
By absolute convergence, the product expands as a Cauchy product: the coefficient of $e^{\pi i \ell z}$ in $f_1 f_2$ is $c(\ell) = \sum_{m + n = \ell} c_1(n) c_2(m)$, a sum of $\ell - n_0(f_1) - n_0(f_2) + 1$ terms each of size $O(\ell^{k_1 + k_2})$, so $c(\ell) = O(\ell^{k_1 + k_2 + 1})$. The lowest-order term arises exactly once, from $(n, m) = (n_0(f_1), n_0(f_2))$.
\end{proof}
...
The bound appearing in Lemma~\ref{lem:bound-I1-I3-I5} decays faster than any polynomial as $r \to \infty$; this is a consequence of standard facts about the $\Gamma$ function. We state the lemma for an arbitrary exponent $\alpha > 0$, so that it also applies (with $\alpha = \pi$) to the analogous bounds for $b$ in Lemma~\ref{lemma:bound-J1-J3-J5} below.

\begin{lemma}\label{lem:integral-bound}
    For all $n \in \N$ and $\alpha > 0$, there exists a constant $C'$ such that for all $r \geq 0$,
    \begin{align}
        r^n \cdot \int_{1}^{\infty} e^{-\alpha s} \, e^{-\pi r /s} \, \dd s \leq C'
    \end{align}
\end{lemma}
\begin{proof}
    ...
    \begin{align}
        r^n \cdot \int_{1}^{\infty} e^{-\alpha s} \, e^{-\pi r /s} \, \dd{s}
        = \int_{1}^{\infty} e^{-\alpha s} \, \left(\abs{r}^n \cdot e^{-\pi r /s}\right) \, \dd{s}
        \leq C \int_{1}^{\infty} e^{-\alpha s} \, s^n \, \dd{s}
    \end{align}
    The $\Gamma$ function is given by
    \begin{align}
        \Gamma(x) = \int_{0}^{\infty} e^{-u} \, u^{x-1} \, \dd{u}
    \end{align}
    for all $x > 0$ (in Mathlib, \verb|Real.Gamma_eq_integral|). Hence, writing $u = \alpha s$ and using $\Gamma(n+1) = n!$ (\verb|Real.Gamma_nat_eq_factorial|), we get
    \begin{align}
        C \int_{1}^{\infty} e^{-\alpha s} \, s^n \, \dd{s}
        \leq C \int_{0}^{\infty} e^{-\alpha s} \, s^n \, \dd{s}
        = C \int_{0}^{\infty} \frac{1}{\alpha^{n+1}} e^{-u} \, u^n \, \dd{u}
        = \frac{C}{\alpha^{n+1}} \Gamma(n+1)
        = \frac{C \cdot n!}{\alpha^{n+1}}
    \end{align}
    Defining $C' := \frac{C \cdot n!}{\alpha^{n+1}}$ finishes the proof.
\end{proof}
...
% AUDIT NOTE: the Lean definition `MagicFunction.b.RealIntegrals.J₆'` currently carries the
% factor +2 instead of the -2 below. The -2 convention used here is the one consistent with the
% Lean statements `perm_J₅ : 𝓕 J₅ = -J₆` and `b_zero : b 0 = 0`, as well as with
% Proposition \ref{prop:b-double-zeros}; the Lean definition should be corrected accordingly.
...
First, suppose that $r>\sqrt{2}$. Then by Proposition~\ref{prop:b-double-zeros}, parametrising $z = it$ (so that $\dd z = i\,\dd t$),
$$b(r)=-4i\,\sin(\pi r^2/2)^2\,\int\limits_{0}^{\infty}\psi_I(it)\,e^{-\pi r^2 t}\,dt. $$
(Note the sign: unlike in Proposition~\ref{prop:a-another-integral}, there is no factor $z^2 = -t^2$ in the integrand to absorb the $i$ coming from $\dd z$.)
