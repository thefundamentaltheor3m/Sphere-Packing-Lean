...
\begin{align}
    \left|\int\limits_{-1}^{i}\phi_0\Big(\frac{-1}{z+1}\Big)\,(z+1)^2\,e^{\pi i r^2 z}\,dz\right| &=\left|\int\limits_{i\infty}^{-1/(i+1)}\phi_0(z)\,z^{-4}\,e^{\pi i r^2 (-1/z-1)}\,\dd z\right| \notag\\
    &= \left|\int_{1/2}^{\infty} \phi_0\left(-\frac{1}{2} + it\right) \left(- \frac{1}{2} + it\right)^{-4} e^{\pi i r^2 \left(\frac{1 - 4t^2}{1 + 4t^2} + i \frac{4t}{1 + 4t^2}\right)}\,\dd t\right| \\
    &\le \int\limits_{1/2}^{\infty}C_0 e^{-2\pi t} \cdot \left(\frac{1}{2}\right)^{-4}\cdot e^{-\pi r^2 \frac{4t}{1 + 4t^2}}\,\dd t \notag \\
    &\leq C\int\limits_{0}^{\infty}e^{-2\pi t}\,e^{-\pi r^2/t}\,\dd t
\end{align}
where $C = 16 C_0$.
The last integral can be expressed in terms of the modified Bessel function $K_1$, which has exponential decay and hence so does the integral.
Instead of appealing the fact, we follow more elementary argument by Slipper \cite[Proposition 4.4.3]{Slipper}.
The integral can be break up into two parts as
\[
\int_0^\infty e^{-2 \pi t} e^{\pi r^2 / t} \dd t = \int_0^{r / \sqrt{2}} e^{-\pi (2t + r^2 / t)} \dd t + \int_{r / \sqrt{2}}^{\infty} e^{-\pi (2t + r^2 / t)} \dd t
\]
and each part can be bounded as
\begin{align}
    \int_0^{r /\sqrt{2}} e^{-\pi(2t + r^2 / t)} \dd t &\le \int_0^{r / \sqrt{2}} e^{-\pi r^2 / t} \dd t = \int_{\sqrt{2} / r}^{\infty} e^{-\pi r^2 t} \frac{\dd t}{t^2} \dd t \le \frac{r^2}{2} \int_{\sqrt{2} / r}^{\infty} e^{-\pi r^2 t} \dd t = \frac{e^{-\sqrt{2} \pi r}}{2 \pi}, \\
    \int_{r / \sqrt{2}}^{\infty} e^{-\pi (2t + r^2 / t)} \dd t &\le \int_{r / \sqrt{2}}^{\infty} e^{-2 \pi t} \dd t = \frac{e^{-\sqrt{2} \pi r}}{2 \pi},
\end{align}
so the entire integral is bounded by $e^{-\sqrt{2} \pi r} / \pi$, which decays exponentially as $r \to \infty$.
The other integrals \eqref{eqn:a-definition} can be treated similarly.
\end{proof}
