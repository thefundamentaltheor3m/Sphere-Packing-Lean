...
The contours themselves are encoded once and for all by explicit parametrisations.
\begin{definition}\label{def:contour-parametrisations}\lean{MagicFunction.Parametrisations.z₁', MagicFunction.Parametrisations.z₂', MagicFunction.Parametrisations.z₃', MagicFunction.Parametrisations.z₄', MagicFunction.Parametrisations.z₅', MagicFunction.Parametrisations.z₆'}\leanok
Define the six contour parametrisations
\begin{align}
    z_1(t) &:= -1 + it, & z_2(t) &:= -1 + t + i, & z_3(t) &:= 1 + it, \notag\\
    z_4(t) &:= 1 - t + i, & z_5(t) &:= it \quad (t \in [0,1]), & z_6(t) &:= it \quad (t \in [1,\infty)), \notag
\end{align}
extended to all of $\R$ by clamping to the endpoint values (\texttt{IccExtend} / \texttt{IciExtend}).
All integrals defining $a$ and $b$ below are stated over these parametrisations, and their basic
bounds ($\Im z_2 = \Im z_4 = 1$, $\|z_5\| \le 1$, $\|z_k\| \le 2$) are recorded once in the
formalization and reused throughout.
\end{definition}

\begin{definition}\label{def:a-definition}\lean{MagicFunction.a.RealIntegrals.a',MagicFunction.a.RadialFunctions.a}\leanok
\uses{def:phi4-phi2-phi0, def:contour-parametrisations}
...
\begin{corollary}\label{cor:phi0-bound}\uses{thm:ramanujan-formula, lemma:mod-div-disc-bound, lemma:Ek-Fourier, prop:A_E-tsum}\lean{MagicFunction.PolyFourierCoeffBound.norm_φ₀_le}\leanok
...
\begin{lemma}\label{lem:integral-bound}\lean{MagicFunction.a.Schwartz.I1Decay.xpow_integral_le_of_Cpow}\leanok
...
\begin{lemma}\label{lem:bound-I1-I3-I5}\lean{MagicFunction.a.SchwartzProperties.I₁'_decay', MagicFunction.a.SchwartzProperties.I₃'_decay', MagicFunction.a.SchwartzProperties.I₅'_decay'}\leanok
...
\begin{lemma}\label{lem:bound-I2-I4-I6}\lean{MagicFunction.a.SchwartzProperties.I₂'_decay', MagicFunction.a.SchwartzProperties.I₄'_decay', MagicFunction.a.SchwartzProperties.I₆'_decay'}\leanok
...
\begin{lemma}[Smoothness of the radial profiles of $a$]\label{lemma:I-smooth}\uses{def:a-definition, thm:contDiff-integral-Ioo01, lemma:hasDerivAt-integral-IciOne, thm:contDiffOn-family-hasDerivAt}\lean{MagicFunction.a.SchwartzProperties.I₁'_smooth', MagicFunction.a.SchwartzProperties.I₂'_smooth', MagicFunction.a.SchwartzProperties.I₃'_smooth', MagicFunction.a.SchwartzProperties.I₄'_smooth', MagicFunction.a.SchwartzProperties.I₅'_smooth', MagicFunction.a.SchwartzProperties.I₆'_smooth'}\leanok
  The radial profiles $I_1', \dots, I_5' : \R \to \C$ are $C^\infty$ on $\R$, and $I_6'$
  multiplied by the cutoff of Definition~\ref{def:radial-cutoff} is $C^\infty$ on $\R$.
\end{lemma}
\begin{proof}\leanok
  Each $I_k'$ is a parametric integral of the form treated in
  Section~\ref{sec:integration-infrastructure}: for $I_1'$--$I_5'$ (integrals over $(0,1)$),
  differentiation under the integral sign on $(0,1)$
  (Theorem~\ref{thm:contDiff-integral-Ioo01}) applies, using the boundedness of the integrands on
  the corresponding contours; $I_3'$ and $I_5'$ reduce to $I_1'$ via multiplication by
  $e^{2\pi i x}$ resp.\ $-2e^{\pi i x}$. For $I_6'$ (an integral over $[1,\infty)$),
  differentiation under the integral sign on $[1,\infty)$
  (Lemma~\ref{lemma:hasDerivAt-integral-IciOne}) gives smoothness on $(-2, \infty)$, which
  suffices after multiplying by the cutoff.
\end{proof}

\begin{proposition}\label{prop:a-schwartz}\lean{MagicFunction.FourierEigenfunctions.a}\leanok
\uses{def:a-definition, cor:phi0-bound, lemma:I-smooth, def:fCutSchwartz, thm:schwartz-multidimensional}
$a(x)$ is a Schwartz function.
\end{proposition}
\begin{proof}\leanok
\uses{lem:bound-I1-I3-I5, lem:bound-I2-I4-I6, lem:integral-bound}
It suffices to show that the function is smooth and decays faster than any polynomial.
The smoothness follows from Lemma~\ref{lemma:I-smooth}.
...
Each profile $I_k'$ is packaged as a one-dimensional Schwartz function via
Definition~\ref{def:fCutSchwartz} and lifted to $\R^8$ by
Theorem~\ref{thm:schwartz-multidimensional}.
\end{proof}

\begin{proposition}\label{prop:a-fourier}\uses{lemma:Ek-Fourier, def:E2, def:a-definition, lemma:Gaussian-Fourier, prop:a-schwartz, thm:perm-contour-pos, lemma:inv-change-of-variables}\lean{MagicFunction.a.Fourier.eig_a}\leanok
...
\begin{corollary}\label{cor:phi0-near-0-infty}\uses{cor:phi0-bound, cor:phi2-bound, cor:phi4-bound, lemma:phi0-transform}\lean{MagicFunction.a.Schwartz.I1Decay.norm_φ₀''_le, MagicFunction.g.CohnElkies.IntegralReps.norm_phi0S_mul_sq_le}\leanok
We have
\begin{align}
...
The passage from the Laplace-type representation of Proposition~\ref{prop:a-double-zeros} to this
formula is carried out in two steps in the formalization: the contour integrands are first
rewritten on shifted vertical lines and on the imaginary axis, and the resulting Laplace
integrals are then evaluated term by term after subtracting the singular parts.

\begin{lemma}\label{lemma:a-vertical-rewrites}\uses{prop:a-double-zeros, def:phi4-phi2-phi0}\lean{MagicFunction.g.CohnElkies.IntegralReps.Φ₁'_shift_left, MagicFunction.g.CohnElkies.IntegralReps.Φ₃'_shift_right, MagicFunction.g.CohnElkies.IntegralReps.Φ₅'_imag_axis_eq_neg_aLaplaceIntegrand, MagicFunction.g.CohnElkies.IntegralReps.Φ_finite_difference_imag_axis}\leanok
The contour integrands appearing in the Laplace representation of $a$ can be rewritten on the
positive imaginary axis: the integrands on the two shifted vertical lines $\pm 1 + it$ differ
from the integrand on the imaginary axis by explicit exponential factors, and their combination
on the imaginary axis equals the subtracted-singularity integrand of
\eqref{eqn:a-another-integral}.
\end{lemma}
\begin{proof}\leanok
Direct computation from the definitions, using the periodicity $z \mapsto z + 2$ of the
integrands and the transformation laws of $\phi_0$, $\phi_{-2}$, $\phi_{-4}$.
\end{proof}

\begin{lemma}\label{lemma:a-laplace-integrable}\uses{lemma:a-vertical-rewrites, cor:disc-nonvanishing}\lean{MagicFunction.g.CohnElkies.IntegralReps.exists_inv_Delta_bound_exp, MagicFunction.g.CohnElkies.IntegralReps.exists_phi2'_phi4'_bound_exp, MagicFunction.g.CohnElkies.IntegralReps.integrableOn_Φ₅'_imag_axis, MagicFunction.g.CohnElkies.IntegralReps.aLaplaceIntegral_convergent}\leanok
There are constants $C, c > 0$ such that $|1/\Delta(it)| \le C e^{2\pi t}$ and the profiles
$\phi_{-2}$, $\phi_{-4}$ satisfy exponential bounds of the same type on $[1, \infty)$.
Consequently, the subtracted-singularity integrand
\begin{equation*}
    t \mapsto t^2\,\phi_0\Big(\frac{i}{t}\Big)-\frac{36}{\pi^2}\,e^{2\pi t}+\frac{8640}{\pi}\,t-\frac{18144}{\pi^2}
\end{equation*}
is integrable against $e^{-\pi r^2 t}$ on $(0, \infty)$ for every $r \ge 0$.
\end{lemma}
\begin{proof}\leanok
The exponential bounds follow from the $q$-expansions; integrability follows by comparison.
\end{proof}

\begin{proposition}\label{prop:a-another-integral}\uses{prop:a-double-zeros, def:phi4-phi2-phi0, lemma:phi0-transform, def:a-definition, thm:rect-deform-tendsto-top, lemma:a-vertical-rewrites, lemma:a-laplace-integrable}\lean{MagicFunction.g.CohnElkies.IntegralReps.aRadial_eq_another_integral_main}\leanok
For $r\geq0$ we have
...
\begin{definition}\label{def:b-definition}\uses{def:psiI-psiT-psiS, def:contour-parametrisations}\lean{MagicFunction.b.RealIntegrals.b',MagicFunction.b.RadialFunctions.b}\leanok
...
\begin{lemma}\label{lemma:psi-bound}\uses{def:resToImagAxis}\lean{MagicFunction.b.PsiBounds.exists_bound_norm_ψS_resToImagAxis_Ici_one}\leanok
...
\begin{lemma}\label{lemma:bound-J1-J3-J5}\lean{MagicFunction.b.Schwartz.J1Smooth.decay_J₁', MagicFunction.b.Schwartz.J3Smooth.decay_J₃', MagicFunction.b.Schwartz.J5Smooth.decay_J₅'}\leanok
...
\begin{lemma}\label{lemma:bound-J2-J4-J6}\lean{MagicFunction.b.Schwartz.J2Smooth.decay_J₂', MagicFunction.b.Schwartz.J4Smooth.decay_J₄', MagicFunction.b.Schwartz.J6Smooth.decay_J₆'}\leanok
...
Combining Lemmas \ref{lemma:bound-J1-J3-J5}, \ref{lemma:bound-J2-J4-J6}, and the radial bridge above, we can prove that $b(x)$ is a Schwartz function.

\begin{lemma}[Smoothness of the radial profiles of $b$]\label{lemma:J-smooth}\uses{def:b-definition, thm:contDiff-integral-Ioo01, lemma:hasDerivAt-integral-IciOne, thm:contDiffOn-family-hasDerivAt}\lean{MagicFunction.b.Schwartz.J1Smooth.contDiff_J₁', MagicFunction.b.Schwartz.J2Smooth.contDiff_J₂', MagicFunction.b.Schwartz.J3Smooth.contDiff_J₃', MagicFunction.b.Schwartz.J4Smooth.contDiff_J₄', MagicFunction.b.Schwartz.J5Smooth.contDiff_J₅', MagicFunction.b.SchwartzIntegrals.J₆'}\leanok
  The radial profiles $J_1', \dots, J_5' : \R \to \C$ are $C^\infty$ on $\R$, and $J_6'$
  multiplied by the cutoff of Definition~\ref{def:radial-cutoff} is $C^\infty$ on $\R$.
\end{lemma}
\begin{proof}\leanok
  As in Lemma~\ref{lemma:I-smooth}: $J_1'$--$J_5'$ are parametric integrals over $(0,1)$ handled
  by differentiation under the integral sign on $(0,1)$
  (Theorem~\ref{thm:contDiff-integral-Ioo01}), using the bounds on $\psi_I, \psi_T$ along the
  contours; $J_6'$ is an integral over $[1,\infty)$ handled by
  Lemma~\ref{lemma:hasDerivAt-integral-IciOne} together with the exponential decay of $\psi_S$ on
  the imaginary axis.
\end{proof}

\begin{proposition}\label{prop:b-schwartz}\uses{lemma:psi-bound, thm:open-rect-deform, lemma:J-smooth, def:fCutSchwartz, thm:schwartz-multidimensional}\lean{MagicFunction.FourierEigenfunctions.b}\leanok
$b(x)$ is a Schwartz function.
\end{proposition}
\begin{proof}\leanok
\uses{lemma:bound-J1-J3-J5, lemma:bound-J2-J4-J6}
Similar to the proof of \ref{prop:a-schwartz}.
\end{proof}

\begin{proposition}\label{prop:b-fourier}\uses{def:b-definition, lemma:Gaussian-Fourier, def:psiI-psiT-psiS, prop:b-schwartz, thm:perm-contour-neg, lemma:inv-change-of-variables}\lean{MagicFunction.b.Fourier.eig_b}\leanok
...
\begin{corollary}\label{cor:psiI-near-0-infty}\uses{lemma:psi-bound, def:psiI-psiT-psiS}\lean{MagicFunction.b.Schwartz.J5Smooth.ψI'_z₅'_eq, MagicFunction.g.CohnElkies.IntegralReps.exists_ψI_bound_exp}\leanok
We have
\begin{align}
...
As for $a$, the passage from Proposition~\ref{prop:b-double-zeros} to this formula requires
controlling the integrand $\psi_I(it)$ on the imaginary axis. This is done through the
$q$-expansions of the theta functions and an explicit cancellation estimate.

\begin{lemma}\label{lemma:b-theta-axis-bounds}\uses{def:H2-H3-H4, def:resToImagAxis, prop:H2-fourier, prop:H3-fourier, prop:H4-fourier}\lean{MagicFunction.g.CohnElkies.AnotherIntegral.B.ThetaAxis.exists_bound_norm_H2_resToImagAxis_sub_two_terms_Ici_one, MagicFunction.g.CohnElkies.AnotherIntegral.B.ThetaAxis.exists_bound_norm_H3_add_H4_resToImagAxis_sub_two_sub_main_Ici_one, MagicFunction.g.CohnElkies.AnotherIntegral.B.ThetaAxis.exists_bound_norm_H4_resToImagAxis_sub_two_terms_Ici_one}\leanok
On $[1, \infty)$ the restrictions $t \mapsto H_i(it)$ agree with the leading terms of their
Fourier expansions up to exponentially small errors: there are constants $C, c > 0$ with
\begin{equation*}
    \big|H_2(it) - 16 e^{-\pi t} - 64 e^{-2\pi t}\big| \le C e^{-(2 + c)\pi t},
    \qquad
    \big|H_4(it) - 1 + 8 e^{-\pi t}\big| \le C e^{-(1 + c)\pi t},
\end{equation*}
and similarly for $H_3$ and for the combination $H_3 + H_4$.
\end{lemma}
\begin{proof}\leanok
Immediate from the Fourier expansions of Propositions~\ref{prop:H2-fourier}--\ref{prop:H4-fourier},
estimating the tails by geometric series.
\end{proof}

\begin{lemma}\label{lemma:b-psiI-cancellation}\uses{lemma:b-theta-axis-bounds, def:psiI-psiT-psiS}\lean{MagicFunction.g.CohnElkies.AnotherIntegral.B.PsiICancellation.psiI'_mul_I_eq_resToImagAxis, MagicFunction.g.CohnElkies.AnotherIntegral.B.PsiICancellation.exists_bound_norm_psiI'_mul_I_sub_exp_add_const_Ici_one, MagicFunction.g.CohnElkies.IntegralReps.bAnotherBase_integrable_exp}\leanok
There are constants $C, c > 0$ such that
\begin{equation*}
    \big|\psi_I(it) - e^{2\pi t} - 144\big| \le C e^{(2 - c)\pi t}
    \qquad (t \ge 1).
\end{equation*}
Consequently, $t \mapsto \psi_I(it) - e^{2\pi t} - 144$ is integrable against $e^{-\pi r^2 t}$
on $(0, \infty)$ for every $r \ge 0$.
\end{lemma}
\begin{proof}\leanok
The function $\psi_I$ is a quotient of polynomial expressions in $H_2, H_3, H_4$ divided by
powers of theta functions; substituting the expansions of Lemma~\ref{lemma:b-theta-axis-bounds}
and tracking the leading terms, the singular parts $e^{2\pi t}$ and $144$ cancel, leaving an
exponentially decaying remainder. Integrability follows by comparison.
\end{proof}

\begin{proposition}\label{prop:b-another-integral}\uses{prop:b-double-zeros, def:b-definition, lemma:b-theta-axis-bounds, lemma:b-psiI-cancellation}\lean{MagicFunction.g.CohnElkies.IntegralReps.bRadial_eq_another_integral_main}\leanok
For $r\geq0$ we have
