...
\end{theorem}
\begin{proof}\leanok
Consider the periodization
\[
    F(v) := \sum_{\ell \in \Lambda} f(\ell + v).
\]
Since $f$ is Schwartz, the sum converges absolutely and locally uniformly, so $F$ is a
continuous $\Lambda$-periodic function on $\R^d$. Its Fourier coefficients with respect to the
torus $\R^d / \Lambda$ are, for $m \in \Lambda^*$,
\[
    c_m = \frac{1}{\Vol{\R^d/\Lambda}} \int_{\R^d/\Lambda} F(v)\, e^{-2\pi i \ang{v, m}}\, dv
        = \frac{1}{\Vol{\R^d/\Lambda}} \int_{\R^d} f(v)\, e^{-2\pi i \ang{v, m}}\, dv
        = \frac{\widehat{f}(m)}{\Vol{\R^d/\Lambda}},
\]
where the middle equality follows by unfolding the sum over $\Lambda$ (Fubini, justified by
absolute convergence). Since $\widehat{f}$ is again Schwartz, the coefficients $c_m$ are
absolutely summable over $\Lambda^*$, so the Fourier series of $F$ converges absolutely and
uniformly to $F$ itself:
\[
    \sum_{\ell\in\Lambda}f(\ell + v) = F(v)
    = \sum_{m\in\Lambda^*} c_m\, e^{2\pi i \ang{v, m}}
    = \frac{1}{\Vol{\R^d/\Lambda}} \sum_{m\in\Lambda^*}\widehat{f}(m)\, e^{2\pi i \ang{v, m}}.
\]
In the formalization this is \texttt{SchwartzMap.poissonSummation\_lattice}, built on Mathlib's
...
In the formalization, we package smooth, fast-decaying radial profiles as Schwartz functions on $\R$ via \texttt{RadialSchwartz.Bridge.fCutSchwartz}, then lift them to $\R^d$ via composition with $\|x\|^2$ using \texttt{schwartzMap\_multidimensional\_of\_schwartzMap\_real}.
