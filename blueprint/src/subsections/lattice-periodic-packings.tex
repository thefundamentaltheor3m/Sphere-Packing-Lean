...
\end{theorem}
\begin{proof}\leanok
By scaling (\Cref{lemma:scaling-density}) both constants may be computed using only packings of
separation $1$.

Since every periodic packing is in particular a packing, the periodic constant is at most the
sphere packing constant.

For the reverse inequality it suffices to show that for every packing $\mathcal{P}$ of
separation $1$ and every $a$ less than the density of $\mathcal{P}$ there is a \emph{periodic}
packing of separation $1$ whose density exceeds $a$. Choose $R$ large enough that the finite
density of $\mathcal{P}$ over the cube $[-R, R)^d$ exceeds $a$; let $X$ be the set of centers of
$\mathcal{P}$ lying in the cube whose balls do not meet the boundary, and repeat $X$ periodically
with respect to the lattice $(2R)\Z^d$. Distinct translates of the cube are far enough apart that
the repeated configuration still has separation $1$, and its density equals the finite density of
the truncated configuration, which exceeds $a$ for $R$ large. Taking the supremum over $a$
finishes the proof.
\end{proof}
