...
\begin{definition}\label{EuclideanLattice.isLattice}\lean{EuclideanLattice.isLattice}\leanok
  % A lattice in the Euclidean space $\R^d$ is a discrete, co_compact, abelian subgroup.
  We say that an additive subgroup $\Lambda \leq \R^d$ is a \emph{lattice} if it is discrete and its $\R$_span contains all the elements of $\R^d$.
\end{definition}
...
\todo{Remove this, since it's a duplicate of section 2.3 (Density of periodic packings.}
...
\begin{definition}\label{def_Periodic_sphere_packing_constant}\uses{SpherePacking.Density, SpherePacking.PeriodicPackingCentres}\notready
...
\begin{theorem}\label{periodic_packing_optimal}\uses{SpherePacking.Density, def_Periodic_sphere_packing_constant}\notready
