...
\begin{proposition}\label{prop: b(r) Fourier}\uses{def:b-definition, lemma:Gaussian-Fourier, def: psi I psi T psi S}
The function $b$ defined by \eqref{eqn:b-definition} belongs to the Schwartz space and satisfies
  $$\widehat{b}(x)=-b(x). $$
\end{proposition}
\begin{proof}
Here, we repeat the arguments used in the proof of Proposition~\ref{prop:a-fourier}. First we show that $b$ is a Schwartz function. We have
\begin{align}
...
Thus, as in the proof of Proposition~\ref{prop:a-fourier} we estimate the first summand in the left-hand side of~\eqref{eqn:b-definition}
