...
\begin{lemma}\label{lemma:phi0-bound}\uses{thm:ramanujan-formula, lemma:disc-prodformula, lemma:Ek-Fourier}
...
We need bounds for $\phi_{-2}$ and $\phi_{-4}$ in order to control $\phi_0$ near $0$ and $i\infty$.
...
\begin{lemma}\label{lemma:phi-2-phi-4-bound}\uses{def:phi4-phi2-phi0, lemma:disc-prodformula, lemma:Ek-Fourier}
There exist constants $C_{-2}, C_{-4}>0$ such that
\begin{align}
    |\phi_{-2}(z)| &\le C_{-2} \label{eqn:phi-2-bound} \\
    |\phi_{-4}(z)| &\le C_{-4} e^{2 \pi \Im z} \label{eqn:phi-4-bound}
\end{align}
for all $z$ with $\Im z > 1/2$.
\end{lemma}
\begin{proof}
The proof is similar to the proof of Lemma~\ref{lemma:phi0-bound}.
Indeed, we can take $C_{-2}$ and $C_{-4}$ as
\begin{align}
    C_{-2} &= 3 \cdot \frac{E_4(i/2) E_4'(i/2)}{\Delta(i/2)} \\
    C_{-4} &= e^{-\pi} \frac{E_4(i/2)^{2}}{\Delta(i/2)}.
\end{align}
\end{proof}

\begin{corollary}\label{cor:phi0-near-0-infty}\uses{lemma:phi-2-phi-4-bound, lemma:phi0-bound, lemma:phi0-transform}
We have
\begin{align}
...
For the second estimate, by \eqref{eqn:phi0-trans-S}, \eqref{eqn:phi-2-bound}, and \eqref{eqn:phi-4-bound}, we have
\begin{equation}
...
\uses{cor:phi0-near-0-infty, def:a-definition}
...
where
\begin{equation}
    C_S = \frac{\sum_{n \ge 3} |a_n| e^{-\pi (n - 3) / 2}}{2\prod_{n \ge 1} (1 - e^{\pi n})^{24}}.
\end{equation}
...
\begin{lemma}\label{lemma:psiI-bound}\uses{lemma:psiS-bound, lemma:psiS-new, lemma:psi fourier I psi fourier T psi fourier S}
...
\begin{proposition}\label{prop: b(r) double zeroes}\uses{lemma: psi fourier I psi fourier T psi fourier S, def:psiI-psiT-psiS}
