...
\begin{proposition}\label{prop:ineqA}\uses{lemma:ineqABnew-equiv, lemma:F-G-phi-psi-identities, lemma:F-G-pos, cor:ineqAnew}\lean{MagicFunction.g.CohnElkies.g_nonpos_of_norm_sq_ge_two}\leanok
...
\begin{proposition}\label{prop:ineqB}\uses{lemma:ineqABnew-equiv, lemma:F-G-phi-psi-identities, cor:ineqBnew}\lean{MagicFunction.g.CohnElkies.fourier_g_nonneg}\leanok
...
\begin{lemma}\label{lemma:F-G-phi-psi-identities}\uses{def:FG-definition, lemma:lv1-lv2-identities, lemma:psi-new}\lean{φ₀,F,G,MagicFunction.b.PsiBounds.ψS_apply_eq_factor,Delta_eq_H₂_H₃_H₄}\leanok
We have
\begin{align}
...
\begin{lemma}\label{lemma:F-G-pos}\uses{thm:ramanujan-formula, cor:theta-pos, def:resToImagAxis}\lean{F_pos, G_pos}\leanok
...
\begin{corollary}\label{cor:MLDE-pos}\uses{lemma:FG-de, cor:disc-pos, cor:theta-pos, def:E2, lemma:der-q-series}\lean{SerreDer_22_L₁₀_pos}\leanok
...
\begin{lemma}\label{lemma:Qlim}\uses{lemma:E2-transform-S, lemma:Ek-is-modular-form, lemma:theta-transform-S-T, prop:A_E-tsum}\lean{FmodG_rightLimitAt_zero}\leanok
We have
\begin{equation}
...
The monotonicity argument below also needs to know that $\mathcal{L}_{1, 0} := F'G - FG'$ is
eventually positive on the imaginary axis; this is extracted from the vanishing orders of $F$
and $G$ at $i\infty$.
\begin{lemma}\label{lemma:L10-eventually-pos}\uses{def:FG-definition, def:resToImagAxis}\lean{L₁₀_eventuallyPos}\leanok
The function $t \mapsto \mathcal{L}_{1,0}(it) = (F'G - FG')(it)$ is positive for all
sufficiently large $t$.
\end{lemma}
\begin{proof}\leanok
The vanishing orders of $F$ and $G$ at $i\infty$ are $2$ and $\frac{3}{2}$ respectively, so
$\mathcal{L}_{1,0}/(FG) = F'/F - G'/G \to 2 - \frac{3}{2} = \frac{1}{2} > 0$ as $t \to \infty$,
and $F(it)G(it) > 0$.
\end{proof}

\begin{proposition}\label{prop:Qdec}\uses{thm:ramanujan-formula, thm:serre-der-prod-rule, cor:MLDE-pos, thm:anti-serre-der-pos, lemma:L10-eventually-pos}\lean{FmodG_strictAntiOn}\leanok
...
\begin{theorem}\label{thm:g1}\uses{prop:a-fourier, prop:b-fourier, prop:a-double-zeros, prop:b-double-zeros, prop:ineqA, prop:ineqB, prop:a0, prop:b0}\lean{MagicFunction.g.CohnElkies.g_nonpos_of_norm_sq_ge_two,MagicFunction.g.CohnElkies.fourier_g_nonneg,g_zero,fourier_g_zero}\leanok
The function
