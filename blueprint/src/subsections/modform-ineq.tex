...
\begin{definition}\label{def:FG-definition}\uses{def:E2, def:Ek, def:H2-H3-H4}
...
\begin{lemma}\label{lemma:F-G-pos}\uses{thm:ramanujan-formula, cor:theta-pos}
...
\begin{corollary}\label{cor:F-G-de}\uses{lemma:F-G-de, cor:disc-pos, cor:theta-pos, def:E2}
...
\begin{lemma}\label{lemma:Qlim}\uses{lemma:E2-transform-S, lemma:Ek-is-modular-form, lemma:theta-transform-S-T}
We have
...
\begin{proposition}\label{prop:Qdec}\uses{thm:ramanujan-formula, thm:serre-der-prod-rule, cor:F-G-de, thm:anti-serre-der-pos, lemma:log-der-inf}
...
Its Serre derivative $\partial_{22} \mathcal{L}_{1, 0}$ is positive by Corollary \ref{cor:F-G-de}:
\begin{align}
