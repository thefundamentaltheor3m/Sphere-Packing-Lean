...
% \noindent We observe that we can compute the values of $A(t)$ for $t\in(0,\infty)$ with any given precision. Indeed, from identities \eqref{eqn:phi0-trans-S} and \eqref{eqn: psi S define} we obtain the following two presentations for $A(t)$
% \begin{align}
...
\uses{prop:a-double-zeros, prop: b(r) double zeroes, prop:ineqA, prop:ineqB}
The function
