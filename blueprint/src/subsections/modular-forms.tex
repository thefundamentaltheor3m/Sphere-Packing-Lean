...
\begin{lemma}\label{lemma:Ek-Fourier}\uses{def:Ek}\lean{E_k_q_expansion, E4_q_exp, E6_q_exp}\leanok
...
\subsection{q-expansion machinery}

The basic Eisenstein Fourier expansion is enough for the first modular-form statements, and the
lemma above already includes the explicit coefficient formulas for $E_4$ and $E_6$. The later
$\varphi$-bounds also use the convergent tsum expression for the combination $E_2E_4-E_6$. The
supporting Lean development packages the needed summability, product, and termwise-differentiation
lemmas in \texttt{QExpansionLemmas.lean}, \texttt{MultipliableLemmas.lean}, and
	exttt{TsumDerivWithin.lean}.

\begin{lemma}\label{lemma:qexp-infrastructure}\uses{def:Mk}\lean{hasDerivAt_tsum_fun, qExpansion_smul2, qExpansion_ext2, qExpansion_sub1, qExpansion_pow, MultipliableEtaProductExpansion, modularForm_tendsto_atImInfty}\leanok
The $q$-series manipulations used below are justified by the following facts:
a locally uniformly convergent series of holomorphic functions may be differentiated termwise;
the $q$-expansion of level-one modular forms is compatible with scalar multiples, differences,
and powers; and the eta/discriminant product expansions are multipliable on $\mathfrak{H}$.
Moreover a modular form tends to its constant $q$-coefficient as $z \to i\infty$.
\end{lemma}
\begin{proof}\leanok
Standard complex analysis (Weierstrass) for termwise differentiation; direct computations with
the \texttt{qExpansion} API for the algebraic compatibilities; comparison with a geometric
series for multipliability.
\end{proof}

\begin{proposition}\label{prop:A_E-tsum}\uses{lemma:Ek-Fourier, lemma:qexp-infrastructure, lemma:der-q-series}\lean{A_E_eq_tsum}\leanok
The combination $A_E(z) := E_2(z)E_4(z) - E_6(z)$ can be rewritten as a convergent series
\[
    A_E(z) = \sum'_{n : \mathbb{N}} A_{E,\mathrm{term}}(z,n).
\]
This is the q-expansion identity used later in the $\varphi$-bounds.
\end{proposition}

\subsection{Cusp form theory behind the dimension formulas}

The low-weight dimension formulas later in the blueprint rest on three ingredients: a workable cusp-form
predicate, the division-by-$\Delta$ equivalence, and the vanishing of cusp forms below weight $12$.

\begin{definition}\label{def:IsCuspForm}\lean{IsCuspForm}\leanok
A modular form is a cusp form if it lies in the cusp-form submodule.
\end{definition}

\begin{theorem}\label{thm:cuspFormOfSIFTendstoZero}\uses{def:IsCuspForm}\lean{cuspFormOfSIFTendstoZero}\leanok
If a slash-invariant holomorphic function tends to $0$ at $i\infty$, then it defines a cusp form.
\end{theorem}

\begin{lemma}\label{lemma:IsCuspForm-iff-coeffZero}\uses{def:IsCuspForm}\lean{IsCuspForm_iff_coeffZero_eq_zero}\leanok
For level one, a modular form is a cusp form if and only if its constant $q$-coefficient vanishes.
\end{lemma}

\begin{lemma}\label{lemma:cuspform-transport-equivalences}\uses{def:IsCuspForm}\lean{cuspForm_Gamma_one_equiv_SL, modularForm_Gamma_one_equiv_SL}\leanok
The project’s $\Gamma(1)$ cusp forms and modular forms transport canonically to Mathlib’s $\mathcal{S}\mathcal{L}$ versions.
\end{lemma}

\begin{lemma}\label{lemma:Delta-apply-E4-E6}\lean{Delta_apply_eq_one_div_1728_mul_E4_pow_three_sub_E6_sq}\leanok
The discriminant form satisfies $\Delta(z) = (E_4(z)^3 - E_6(z)^2)/1728$.
\end{lemma}

\begin{lemma}\label{lemma:Delta-q-exp-two}\lean{Delta_q_exp_two}\leanok
The second $q$-coefficient of $\Delta$ is $-24$.
\end{lemma}

\begin{theorem}\label{thm:CuspForms-iso-Modforms}\uses{lemma:cuspform-transport-equivalences, lemma:Delta-apply-E4-E6, lemma:Delta-q-exp-two}\lean{CuspForms_iso_Modforms}\leanok
Division by $\Delta$ gives an equivalence $\mathrm{CuspForm}\,\Gamma(1)\,k \simeq \mathrm{ModularForm}\,\Gamma(1)\,(k - 12)$.
\end{theorem}

\begin{theorem}\label{thm:cuspform-weight-lt-12-zero}\uses{lemma:cuspform-transport-equivalences, thm:CuspForms-iso-Modforms}\lean{cuspform_weight_lt_12_zero}\leanok
The cusp-form space vanishes in weights below $12$.
\end{theorem}

\begin{theorem}\label{thm:cuspform-weight-12-finrank-one}\uses{lemma:cuspform-transport-equivalences, lemma:Delta-apply-E4-E6, lemma:Delta-q-exp-two}\lean{cuspform_weight_12_finrank_one}\leanok
The weight-$12$ cusp-form space is one-dimensional.
\end{theorem}

\begin{corollary}\label{cor:IsCuspForm-weight-lt-eq-zero}\uses{lemma:IsCuspForm-iff-coeffZero, thm:cuspform-weight-lt-12-zero}\lean{IsCuspForm_weight_lt_eq_zero}\leanok
A level-one cusp form of weight $<12$ is zero.
\end{corollary}

\begin{corollary}\label{cor:eq-zero-of-tendsto-zero-atImInfty}\uses{cor:IsCuspForm-weight-lt-eq-zero, lemma:IsCuspForm-iff-coeffZero}\lean{eq_zero_of_tendsto_zero_atImInfty}\leanok
Let $G \in M_k(\Gamma(1))$ with $k<12$.
If $G(z) \to 0$ as $\Im z \to \infty$, then $G=0$.
\end{corollary}

These statements are the infrastructure behind the existing theorem \ref{thm:lvl1_dims} and the
corollary \ref{cor:dim-mf} below, which package the low-weight dimension formulas used later in
the modular-form arguments.
...
\begin{lemma}\label{lemma:dedekind_eta_transformation}\lean{ModularForm.eta_comp_eq_csqrt_I_inv}\uses{def:dedekind_eta}\leanok
...
\begin{lemma}\label{lemma:disc-E4E6}\uses{def:disc-definition}\lean{Delta_apply_eq_one_div_1728_mul_E4_pow_three_sub_E6_sq}\leanok
We have
\begin{equation}
...
\begin{theorem}\label{thm:lvl1_dims}\uses{def:holomorphic-modular-form, lemma:disc-E4E6, def:disc-definition, thm:CuspForms-iso-Modforms, thm:cuspform-weight-lt-12-zero, thm:cuspform-weight-12-finrank-one, cor:IsCuspForm-weight-lt-eq-zero}\lean{ModularForm.dimension_level_one}\leanok
...
\begin{proposition}\label{prop:H2-fourier}\uses{def:H2-H3-H4}\uses{lemma:theta-transform-S-T}\lean{H₂}\leanok
...
\begin{proposition}\label{prop:H3-fourier}\uses{def:H2-H3-H4}\uses{lemma:theta-transform-S-T}\lean{H₃}\leanok
...
\begin{proposition}\label{prop:H4-fourier}\uses{def:H2-H3-H4}\uses{lemma:theta-transform-S-T}\lean{H₄}\leanok
...
\begin{lemma}\label{lemma:jacobi-identity}\uses{lemma:theta-modular, cor:dim-mf, thm:cuspFormOfSIFTendstoZero}\lean{jacobi_identity}\leanok
...
\begin{lemma}\label{lemma:lv1-lv2-identities}\uses{lemma:theta-transform-S-T, lemma:theta-modular, lemma:disc-cuspform, prop:H2-fourier, prop:H3-fourier, prop:H4-fourier, thm:cuspFormOfSIFTendstoZero}\lean{SpherePacking.ModularForms.E₄_eq_thetaE4,SpherePacking.ModularForms.E₆_eq_thetaE6,Delta_eq_H₂_H₃_H₄}\leanok
We have
\begin{align}
...
\begin{corollary}\label{cor:theta-pos}\uses{def:H2-H3-H4, lemma:jacobi-identity, lemma:theta-transform-S-T}\lean{H₂_imag_axis_pos,H₄_imag_axis_pos,jacobi_identity}\leanok
...
Many of the positivity and growth estimates in the later sections are statements about the
restriction of a modular object $F : \mathfrak{H} \to \C$ to the positive imaginary axis,
$t \mapsto F(it)$. In the formalization this restriction is packaged as an explicit operator,
together with transfer lemmas that move growth and decay information between the modular
picture (behaviour at $i\infty$) and the real-variable picture (behaviour as $t \to \infty$).

\begin{definition}\label{def:resToImagAxis}\lean{ResToImagAxis, resToImagAxis}\leanok
For $F : \mathfrak{H} \to \C$ define its \emph{restriction to the imaginary axis} as the function
\begin{equation}
    F|_{i\R_{>0}} : \R \to \C, \qquad
    t \mapsto \begin{cases} F(it) & t > 0, \\ 0 & t \le 0.\end{cases}
\end{equation}
We say that $F$ is \emph{real} (resp.\ \emph{positive}) \emph{on the imaginary axis} if
$F(it) \in \R$ (resp.\ $F(it) > 0$) for all $t > 0$.
\end{definition}

\begin{lemma}\label{lemma:resToImagAxis-props}\uses{def:resToImagAxis}\lean{ResToImagAxis.Real, ResToImagAxis.Pos, ResToImagAxis.Differentiable, ResToImagAxis.SlashActionS}\leanok
The class of functions that are real (resp.\ positive) on the imaginary axis is closed under
addition, multiplication, and scalar multiplication by real (resp.\ positive) constants.
If $F$ is holomorphic on $\mathfrak{H}$, then $t \mapsto F(it)$ is differentiable on $(0, \infty)$.
Moreover, the restriction interacts with the slash action of $S$ by
$(F|_{k}S)(it) = (it)^{-k} F(i/t)$ for $t > 0$.
\end{lemma}
\begin{proof}\leanok
All statements follow directly from the definition; differentiability follows from the chain
rule applied to $t \mapsto it$.
\end{proof}

\begin{lemma}\label{lemma:resToImagAxis-decay-transfer}\uses{def:resToImagAxis, lemma:resToImagAxis-props}\lean{isBigO_resToImagAxis_of_isBigO_atImInfty, tendsto_rpow_mul_resToImagAxis_of_isBigO_exp, tendsto_rpow_mul_resToImagAxis_of_fourier_shift}\leanok
Let $F : \mathfrak{H} \to \C$ and $c > 0$. If $F(z) = O(e^{-c \Im z})$ as $\Im z \to \infty$,
then $F(it) = O(e^{-ct})$ as $t \to \infty$, and for every $s \in \R$,
\begin{equation}
    t^{s} F(it) \longrightarrow 0 \qquad (t \to \infty).
\end{equation}
In particular, if $F$ is bounded at $i\infty$ and has vanishing constant Fourier coefficient
(so that $F(z) = O(e^{-2\pi \Im z})$ after shifting off the constant term), the same conclusion
holds with $c = 2\pi$.
\end{lemma}
\begin{proof}\leanok
The first claim is immediate from the definition of $O$-bounds at $i\infty$.
The second follows since polynomial growth is dominated by exponential decay.
The Fourier-shift variant reduces to the first via the $q$-expansion.
\end{proof}

These transfer lemmas are used throughout Section~\ref{sec: g}: the positivity statements for
$F$, $G$, and the theta functions are statements about restrictions to the imaginary axis, and
the asymptotic input for the monotonicity argument (Proposition~\ref{prop:Qdec}) is obtained by
transferring $q$-expansion decay through Lemma~\ref{lemma:resToImagAxis-decay-transfer}.
...
\begin{definition}\label{def:derivative} \lean{Derivative.normalizedDerivOfComplex} \leanok
...
\begin{lemma}\label{lemma:der-q-series}\uses{def:derivative}\lean{D_qexp_tsum}\leanok
...
\begin{theorem}
\label{thm:serre-der-equiv-action}\uses{def:serre-der, def:E2, lemma:E2-transform-general}\lean{serre_D_slash_equivariant, D_slash}\leanok
...
\begin{theorem}\label{thm:ramanujan-formula}\uses{thm:serre-der-modularity, def:serre-der, lemma:E2-transform-general, cor:dim-mf, cor:eq-zero-of-tendsto-zero-atImInfty} \lean{ramanujan_E₂, ramanujan_E₄, ramanujan_E₆, ramanujan_E₂', ramanujan_E₄', ramanujan_E₆'} \leanok
We have
\begin{align}
...
\begin{proposition}\label{prop:theta-der}\uses{def:serre-der, lemma:theta-transform-S-T, lemma:jacobi-identity, thm:cuspFormOfSIFTendstoZero}\lean{serre_D_two_H₂,serre_D_two_H₄}\leanok
We have
\begin{align}
