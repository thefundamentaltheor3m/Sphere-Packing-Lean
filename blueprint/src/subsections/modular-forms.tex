...
\begin{lemma}\label{lemma:mod_form_poly_growth} : Let $\Gamma$ be a finite index subgroup of $\mathrm{SL}_2(\Z)$ and $f \in \mathcal{M}_k(\Gamma)$ be a modular form of weight $k$. Then the Fourier coefficients $a_n(f)$ has a polynomial growth, i.e. $|a_n(f)| = O(n^k)$.
\end{lemma}
...
\begin{corollary}\label{cor:disc-pos}\uses{def:disc-definition}  \leanok \lean{Delta_im_line}
...
\begin{theorem}
\label{thm:serre-der-equiv-action}\uses{def:serre-der, def:E2}
Serre derivative $\partial_{k}$ is equivariant with the slash action of $\mathrm{SL}_{2}(\mathbb{Z})$ in the following sense:
\begin{equation}
    \partial_{k} (F|_{k}\gamma) = (\partial_{k+2} F)|_{k}\gamma, \quad \forall \gamma \in \mathrm{SL}_{2}(\mathbb{Z}).
\end{equation}
\end{theorem}
\begin{proof}
...
\begin{align}
    ...
    &= F'(z) - \frac{ikc}{2 \pi(cz + d)} F(z) - \frac{k}{12} \left(E_2(z) - \frac{6ic}{\pi(cz + d)}\right) F(z) \\
...
\begin{theorem}\label{thm:serre-der-modularity}\uses{def:serre-der, def:holomorphic-modular-form, thm:serre-der-equiv-action}
...
\begin{theorem}\label{thm:ramanujan-formula}\uses{thm:serre-der-modularity, def:serre-der, lemma:E2-transform-general, cor:dim-mf} \lean{ramanujan_E₂, ramanujan_E₄, ramanujan_E₆, ramanujan_E₂', ramanujan_E₄', ramanujan_E₆'}
We have
\begin{align}
...
\begin{theorem}\label{thm:serre-der-prod-rule}\uses{def:serre-der} \lean{serre_D_mul}
