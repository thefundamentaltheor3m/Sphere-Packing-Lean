...
Especially, we can compute the dimension of $M_k(\Gamma)$ explicitly for the congruence subgroups $\Gamma$, using valence formula:

\begin{theorem}[Valence formula]\label{thm:valence-formula}
\todo{TODO: add the precise statement.}
\end{theorem}
Using this, we can compute the dimensions of the following spaces of modular forms:
...
\begin{corollary}\label{cor:dim-mf}\uses{def:Mk, thm:valence-formula}
We have
\begin{align}
...
\end{lemma}
\begin{proof}
Being a modular form of desired weight and level directly follows from those of $E_4$ and $E_6$, and \eqref{eqn:disc-trans-S} immediately follows.
...
\begin{lemma}\label{lemma:disc-prodformula}\uses{def:disc-definition, thm:valence-formula}
We have
...
The following nonvanishing result, which directly follows from \Cref{lemma:disc-prodformula}, will be used in the construction of the magic function.
\begin{corollary}\label{cor:disc-nonvanishing}\uses{lemma:disc-prodformula, thm:valence-formula}
$\Delta(z) \neq 0$ for all $z \in \h$.
\end{corollary}
\begin{proof}
This follows from the product formula, or valence formula.
\end{proof}
