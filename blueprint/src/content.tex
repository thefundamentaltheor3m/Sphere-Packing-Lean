...
\begin{abstract}
  ...
  \begin{definition}\label{SpherePacking.isLatticePacking}\uses{EuclideanLattice.isLattice}\lean{SpherePacking.isLatticePacking}\leanok
  ...
  \begin{definition}\label{EuclideanLattice.E8}\lean{EuclideanLattice.E8}\leanok
  ...
  \end{lemma}
  \begin{definition}\label{SpherePacking.E8}\uses{EuclideanLattice.E8, SpherePacking.isPacking}\lean{SpherePacking.E8}\leanok
  The $E_8$-lattice sphere packing is the packing of unit balls with centers at $\frac{1}{\sqrt{2}}\Lambda_8$.
  \end{definition}
  Our main result is
  \begin{theorem}\label{SpherePacking.Main}\uses{SpherePacking.E8, SpherePacking.Constant, thm-g, thm-Cohn-Elkies}
  ...
  \begin{definition}\label{SpherePacking.isPeriodicPacking}\uses{SpherePacking.isPacking, EuclideanLattice.isLattice}
      A periodic packing is a sphere packing whose set of centres is $\Lambda$-periodic for some lattice $\Lambda$.
  \end{definition}
  ...
  \begin{definition}\label{def-Periodic-sphere-packing-constant}\uses{SpherePacking.Density, SpherePacking.isPeriodicPacking}\notready
  ...
  \begin{proposition}\label{prop: a(r) Fourier}\uses{lemma-Ek-Fourier}\uses{def-E2}\uses{lemma: j Fourier asymptotic}\uses{def: a(r) definition}\uses{lemma-Gaussian-Fourier}
  ...
  \begin{proposition}\label{prop: a another integral}\uses{prop: a(r) double zeroes}\uses{def: phi4 phi2 phi0}\uses{lemma: phi0 transform}\uses{def: a(r) definition}
  For $r\geq0$ we have
  ...
  \uses{lemma: phi fourier4 phi fourier2 phi fourier0}
  ...
  \uses{prop: inequalities A}
  \uses{prop: inequalities B}
  The function
