...
\begin{abstract}
  ...
  \else
  % print
  The web version of this blueprint is available \href{https://thefundamentaltheor3m.github.io/Sphere-Packing-Lean/blueprint/dep_graph_document.html}{here}.
  \fi
  \end{abstract}
...
\begin{definition}\label{def: Sphere Packing}
...
\begin{definition}\label{def: Lattice Packing}\uses{EuclideanLattice.is_lattice}
  If $X$ is a lattice in $\R^d$ then we say that $\mathcal{P}$ is a \emph{lattice sphere packing}.
\end{definition}

\begin{definition}\label{def: finite density}
