...
public lemma coordCube_unique_covers (L : ℝ) (hL : 0 < L) :
    ∀ x, ∃! g : cubeLattice d L hL, g +ᵥ x ∈ coordCube (d := d) L := fun x => by
...
public lemma coordCube_unique_covers_vadd (L : ℝ) (hL : 0 < L)
    ...
    ∀ x, ∃! g : cubeLattice d L hL, g +ᵥ x ∈ v +ᵥ coordCube (d := d) L := by
  intro x
...
public lemma ball_subset_vadd_coordCube_of_mem_vadd_inner {L r : ℝ} (hL : 0 < L)
  ...
  have h :=
    ball_vadd_subset_vadd (d := d) (Λ := cubeLattice d L hL) (D := coordCube (d := d) L)
