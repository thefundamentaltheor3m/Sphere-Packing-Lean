...
/-!
# Periodic packings: periodizing constructions and boundary control.

Contains:
* Periodizing constructions on lattices (`periodizedCenters`, `periodize_to_periodicSpherePacking`).
* Coordinate cubes (`coordCube`, `coordCubeInner`) and the cube lattice (`cubeLattice`).
* Periodic constant approximation (`PeriodicConstantApprox`).
* Boundary control with main result `periodic_constant_eq_constant`.
-/
