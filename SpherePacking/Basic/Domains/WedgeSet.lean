...
/-! # An open convex wedge in the upper half-plane (closure meets ℝ only at 1). -/
...
public lemma convex_wedgeSet : Convex ℝ wedgeSet := by
  have hEq : wedgeSet = {z : ℂ | (0 : ℝ) < z.im} ∩
...
public lemma mem_upperHalfPlane_of_mem_closure_wedgeSet_ne_one
  ...
  have habs0 : |z.re - 1| = 0 :=
    le_antisymm (by simpa [hzIm] using closure_wedgeSet_subset_abs_re_sub_one_le_im hz)
      (abs_nonneg _)
  exact Complex.ext (by simp [sub_eq_zero.mp (abs_eq_zero.mp habs0)]) (by simp [hzIm])
