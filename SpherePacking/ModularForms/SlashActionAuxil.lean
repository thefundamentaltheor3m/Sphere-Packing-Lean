...
theorem α_zpow_val (k : ℤ) : (α ^ k : SL(2, ℤ)).val = !![1, 2 * k; 0, 1] := by
  ...
  simpa [hα, zpow_mul] using ModularGroup.coe_T_zpow (2 * k)
...
lemma Γ2_c_eq_zero (A : Γ 2) (h : A.1 1 0 = 0) : A ∈ Subgroup.closure {α, β, negI} := by
  ...
  obtain ⟨k, hk⟩ : (2 : ℤ) ∣ val.val 0 1 := by
...
lemma Γ2_odd_00 (A : Γ 2) : Odd (A.val.val 0 0) :=
  (ZMod.intCast_eq_one_iff_odd (n := A.val.val 0 0)).1 ((Gamma_mem (N := 2) (γ := A.1)).1 A.2).1

/-- The `(1, 0)` entry of any element of `Γ(2)` is even. -/
lemma Γ2_even_10 (A : Γ 2) : Even (A.val.val 1 0) :=
  even_iff_two_dvd.2 <| by
    simpa [ZMod.intCast_zmod_eq_zero_iff_dvd] using ((Gamma_mem (N := 2) (γ := A.1)).1 A.2).2.2.1
...
lemma Γ2_descent (A : Γ 2) (h : A.1 1 0 ≠ 0) :
  ...
  have ha'_ne : A.val.val 0 0 + 2 * n * A.val.val 1 0 ≠ 0 := by
    rcases ha'_odd with ⟨t, ht⟩; omega
