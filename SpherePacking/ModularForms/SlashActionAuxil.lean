...
-- The following two are bundled in an instance `ModularForm.SLAction` in Mathlib,
-- so we use `SlashAction.slash_mul` for the second attribute.
attribute [blueprint
  "def:slash-operator"
  (statement := /--
  Let $F$ be a function on $\h$ and $\gamma\in\mathrm{SL}_2(\Z)$. Then the \emph{slash operator}
  acts on $F$ by
...
attribute [blueprint
  ...
  (proofUses := [UpperHalfPlane.denom_cocycle])
  (latexEnv := "lemma")]
...
attribute [blueprint
  ...
  $$
    \Gamma(N):=\left\{\left.\left(\begin{smallmatrix}a&b\\c&d\end{smallmatrix}\right)\in\Gamma_1
    \right|\left(\begin{smallmatrix}a&b\\c&d\end{smallmatrix}\right)\equiv
    \left(\begin{smallmatrix}1&0\\0&1\end{smallmatrix}\right)\;\mathrm{mod}\;N\right\}.
  $$
  -/)] Gamma
...
attribute [blueprint
  ...
  (statement := /--
  A subgroup $\Gamma\subset\Gamma_1$ is called a \emph{congruence subgroup} if
  $\Gamma(N)\subset\Gamma$ for some $N\in\N$.
...
@[blueprint "def:Gamma-generators"]
...
attribute [blueprint "def:Gamma-generators"] S T

@[blueprint
  "lemma:Gamma-1-generators"
  (statement := /-- We have $\Gamma(1) = \langle S, T, -I \rangle$. -/)
  (proof := /-- See~\cite[Exercise 1.1.1]{first course}. -/)
  (latexEnv := "lemma")]
...
@[blueprint
  ...
  (proof := /-- See~\cite[Exercise 1.2.4]{first course}. -/)
  (latexEnv := "lemma")]
