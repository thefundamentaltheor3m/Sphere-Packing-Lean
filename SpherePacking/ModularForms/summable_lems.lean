...
theorem int_sum_neg {α : Type*} [AddCommMonoid α] [TopologicalSpace α] [T2Space α] (f : ℤ → α) :
  ∑' d : ℤ, f d = ∑' d, f (-d) :=
  (negEquiv.tsum_eq f).symm

theorem summable_neg {α : Type*} [TopologicalSpace α] [AddCommMonoid α] (f : ℤ → α)
    (hf : Summable f) : Summable fun d => f (-d) := negEquiv.summable_iff.mpr hf
...
theorem nat_pos_tsum2 {α : Type _} [TopologicalSpace α] [AddCommMonoid α]
  ...
  rintro (_ | x) hx
  · exact hf
  · exact (hx ⟨⟨x + 1, x.succ_pos⟩, rfl⟩).elim
...
theorem tsum_pNat {α : Type _} [AddCommGroup α] [UniformSpace α] [IsUniformAddGroup α] [T2Space α]
  ...
  rw [tsum_eq_zero_of_not_summable hf2,
    tsum_eq_zero_of_not_summable ((nat_pos_tsum2 f hf).not.mpr hf2)]
...
theorem nat_pos_tsum2' {α : Type*} [TopologicalSpace α] [AddCommMonoid α] (f : ℕ → α) :
  ...
  exact summable_congr (fun b => by simp)





theorem upp_half_not_ints (z : ℍ) (n : ℤ) : (z : ℂ) ≠ n := z.ne_intCast n


lemma aus (a b : ℂ) : a + b ≠ 0 ↔ a ≠ -b := eq_neg_iff_add_eq_zero.symm.ne

lemma pnat_inv_sub_squares (z : ℍ) :
  (fun n : ℕ+ => 1 / ((z : ℂ) - n) + 1 / (z + n)) = fun n : ℕ+ => 2 * z.1 * (1 / (z ^ 2 - n ^ 2)):=
  by
  funext n
  have h1 : (z : ℂ) - n ≠ 0 := by rw [sub_ne_zero]; exact upp_half_not_ints z n
  have h2 : (z : ℂ) + n ≠ 0 := by rw [aus]; simpa using upp_half_not_ints z (-(n : ℤ))
  have h3 : (z : ℂ) ^ 2 - (n : ℂ) ^ 2 ≠ 0 := by rw [sq_sub_sq]; exact mul_ne_zero h2 h1
  field_simp
  ring
...
lemma upper_half_plane_ne_int_pow_two (z : ℍ) (n : ℤ) : (z : ℂ) ^ 2 - n ^ 2 ≠ 0 := by
  intro h
  rw [sq_sub_sq, mul_eq_zero, add_eq_zero_iff_eq_neg, sub_eq_zero] at h
  rcases h with h | h
  · exact upp_half_not_ints z (-n) (by simpa using h)
  · exact upp_half_not_ints z n h
...
theorem vector_norm_bound (b : Fin 2 → ℤ) (hb : b ≠ 0) (HB1 : b ≠ ![0, -1]) :
  ...
  have ht : b = ![b 0, b 1] := by
    ext i
    fin_cases i <;> rfl
  nth_rw 3 [Real.rpow_of_add_eq (y := -1) (z := -2) (norm_nonneg _) (by norm_num)
    (by norm_num)]
  rw [← mul_assoc]
  apply mul_le_mul
...
@[simp]
lemma δ_eq : δ 0 0 = 1 := rfl

@[simp]
lemma δ_eq2 : δ 0 (-1) = 2 := rfl
...
lemma G_2_alt_summable_δ (z : ℍ) : Summable fun (m : Fin 2 → ℤ) =>
    ...
    rw [← Finset.summable_compl_iff ({ ![0,0], ![0,-1]} : Finset (Fin 2 → ℤ))]
    apply ((G_2_alt_summable z).subtype _).congr
    intro b
    simp only [Fin.isValue, one_div, mul_inv_rev, comp_apply, left_eq_add]
    have hmem : (↑b : Fin 2 → ℤ) ∉ ({![0, 0], ![0, -1]} : Finset (Fin 2 → ℤ)) := b.2
    rw [Finset.mem_insert, Finset.mem_singleton, not_or] at hmem
    obtain ⟨hb1, hb2⟩ := hmem
    simp only [δ, Fin.isValue, Int.reduceNeg]
    split_ifs with h1 h2
    · exact absurd (by funext i; fin_cases i <;> simp [h1.1, h1.2]) hb1
    · exact absurd (by funext i; fin_cases i <;> simp [h2.1, h2.2]) hb2
    · rfl
...
theorem summable_1 (k : ℕ) (z : ℍ) (hk : 1 ≤ k) :
  ...
  apply (Asymptotics.IsBigO.neg_left hl).congr_left
  intro n
  rw [@neg_inv]
  congr
  ring
...
theorem summable_3 (m : ℕ) (y : {z : ℂ | 0 < z.im}) :
  ...
  · apply Summable.add
    · have h0 := summable_1 m (⟨y, y.2⟩ : ℍ) (by omega)
      simp only [Nat.reduceLeDiff, mem_setOf_eq, one_div] at *
      apply h0.subtype
    have h1 := summable_2 m (⟨y, y.2⟩ : ℍ) (by omega)
    simp only [Nat.reduceLeDiff, mem_setOf_eq, one_div] at *
    apply h1.subtype
...
theorem summable_iter_derv' (k : ℕ) (y : ℍ') :
  ...
  conv =>
    ...
    rw [← norm_pow, ← norm_mul,
      show cexp (2 * (↑π * (Complex.I * (↑n * ↑y)))) = cexp (2 * (↑π * (Complex.I * (↑y)))) ^ n by
      rw [← Complex.exp_nsmul]
      congr
      ring]
  apply summable_norm_pow_mul_geometric_of_norm_lt_one
  simpa [← mul_assoc] using exp_upperHalfPlane_lt_one (⟨y, y.2⟩ : ℍ)
...
lemma hsum (k : ℕ) (z : ℍ) : Summable fun b : ℕ+ => ∑ _ ∈ (b : ℕ).divisors, b ^ k *
    ...
    apply Summable.of_nonneg_of_le _ _ (summable_norm_iff.mpr (a33 (k+1) 1 z))
...
theorem exp_series_ite_deriv_uexp'' (k : ℕ) (x : {z : ℂ | 0 < z.im}) :
  ...
  exact tsum_congr fun b => exp_iter_deriv_within k b x.2
    ...
    EqOn
      ...
      (fun x : ℂ =>
        -(2 * ↑π * Complex.I) * ∑' n : ℕ, (2 * ↑π * Complex.I * n) ^ (k : ℕ) * Complex.exp (2 * ↑π *
          Complex.I * n * x))
      {z : ℂ | 0 < z.im} :=
  by intro z hz; exact iter_der_within_add k ⟨z, hz⟩
...
theorem tsum_ider_der_eq (k : ℕ) (x : {z : ℂ | 0 < z.im}) :
  ...
  tsum_congr fun b => by simpa using iter_div_aut_add b k x.2
...
theorem auxp_series_ite_deriv_uexp''' (k : ℕ) :
  ...
  rw [this]
  simpa using tsum_ider_der_eq k ⟨x, hx⟩
...
theorem tsum_aexp_contDiffOn (k : ℕ) :
  ...
  · intro K hK1 hK2
    ...
    have hKK2 : IsCompact K2 := by
      have hK2' : IsCompact (Set.univ : Set K) := isCompact_iff_isCompact_univ.mp hK2
      simpa [K2, toUpper] using hK2'.image htoUpper
    obtain ⟨u, hu1, hu2⟩ := aut_bound_on_comp K2 hKK2 m
    refine ⟨u, hu1, ?_⟩
    intro n s
    have hsK2 : toUpper s ∈ K2 := by
      refine ⟨s, by simp, rfl⟩
...
theorem aux_iter_der_tsum_eqOn (k : ℕ) (hk : 2 ≤ k) :
  ...
  rw [Nat.sub_add_cancel (by linarith : 1 ≤ k)] at this
  norm_cast at *
...
lemma EisensteinSeries_Identity (z : ℍ) :
  ...
  rw [← cot_series_repr z, pi_mul_cot_pi_q_exp z]; ring
...
theorem q_exp_iden (k : ℕ) (hk : 2 ≤ k) (z : ℍ) :
  ...
  rw [← mul_right_inj' hk2, this]
  have h3 : (-1) ^ ((k - 1)) * ↑(k - 1)! * ((-(2 * ↑π * Complex.I)) ^ k / ↑(k - 1)!) = -(2 * ↑π *
    Complex.I) ^ k :=
    by
    rw [mul_div, div_eq_mul_one_div, div_eq_inv_mul]; simp_rw [← mul_assoc]
    simp only [mul_one]
...
lemma G2_summable_aux (n : ℤ) (z : ℍ) (k : ℤ) (hk : 2 ≤ k) :
  ...
  exact_mod_cast linear_bigO_pow n z k
...
theorem a1 (k : ℕ) (e : ℕ+) (z : ℍ) :
  ...
  apply Summable.mul_left
  refine Summable.congr (f := fun c : ℕ ↦ cexp (2 * ↑π * Complex.I * ↑z * ↑↑e) ^ c)
    (summable_geometric_iff_norm_lt_one.mpr ?_)
    (fun c => by simp only []; rw [← Complex.exp_nat_mul]; ring_nf)
