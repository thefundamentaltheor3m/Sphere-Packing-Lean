...
lemma E₂_sub_one_isBigO_exp : (fun z : ℍ => E₂ z - 1) =O[atImInfty]
  ...
  isbigo_im_infty 192 1 with z hz
  ...
  have hsum_le_8q : ‖q‖ / (1 - ‖q‖) ^ 3 ≤ 8 * ‖q‖ :=
    div_one_sub_pow_three_le (norm_nonneg _) hq_lt_half
  have hq_eq_exp : ‖q‖ = Real.exp (-2 * π * z.im) := by rw [Complex.norm_exp]; push_re_im
