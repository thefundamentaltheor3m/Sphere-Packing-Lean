...
private lemma tendsto_E₄_atImInfty : Tendsto (fun z : ℍ => E₄ z) atImInfty (𝓝 (1 : ℂ)) :=
...
/-- The anomaly term `corr γ z = 12·(2πi)⁻¹·(γ₁₀ / (γ₁₀ z + γ₁₁))` appearing in the
quasi-modular transformation law for `E₂`. -/
private noncomputable def ramanujanE2_corr (γ : SL(2, ℤ)) (z : ℍ) : ℂ :=
  (12 : ℂ) * (2 * π * I)⁻¹ * (γ 1 0 / denom γ z)

/-- The correction term `ramanujanE2_corr γ` is mdifferentiable (it is a rational function
of `z` with denominator `denom γ z ≠ 0`). -/
private lemma ramanujanE2_corr_holo (γ : SL(2, ℤ)) :
    MDifferentiable 𝓘(ℂ) 𝓘(ℂ) (ramanujanE2_corr γ) := by
  ...
  refine hG.congr ?_
  intro z hz
  simp [ramanujanE2_corr, Function.comp_apply, ofComplex_apply_of_im_pos hz]

/-- `D(corr γ) = -(1/12)·corr γ · corr γ`: a direct computation from
`deriv (c / (cz + d)) = -c² / (cz + d)²`. -/
private lemma ramanujanE2_corr_D (γ : SL(2, ℤ)) :
    D (ramanujanE2_corr γ) = - 12⁻¹ * (ramanujanE2_corr γ) * (ramanujanE2_corr γ) := by
  funext z
  have h_eq :
      ((ramanujanE2_corr γ) ∘ ofComplex) =ᶠ[nhds (z : ℂ)]
        (fun w : ℂ => (12 : ℂ) * (2 * π * I)⁻¹ * (γ 1 0 / denom γ w)) := by
    filter_upwards [isOpen_upperHalfPlaneSet.mem_nhds z.2] with w hw
    simp [ramanujanE2_corr, Function.comp_apply, ofComplex_apply_of_im_pos hw]
  ...
  dsimp [ramanujanE2_corr]
  field_simp [htwoPiI, hz0]
  ring_nf
  simp

/-- Quasi-modular transformation law: `E₂ ∣[2] γ = E₂ + ramanujanE2_corr γ`. -/
private lemma ramanujanE2_E₂_slash (γ : SL(2, ℤ)) :
    (E₂ ∣[(2 : ℤ)] γ) = E₂ + ramanujanE2_corr γ := by
  rw [E₂_slash]; rfl

/-- Slash formula for `D E₂` at weight 4: it equals `D E₂ + (1/6)·E₂·corr + (1/12)·corr²`. -/
private lemma ramanujanE2_DE2_slash (γ : SL(2, ℤ)) :
    D E₂ ∣[(4 : ℤ)] γ =
      D E₂
        + (6⁻¹ : ℂ) • (E₂ * ramanujanE2_corr γ)
        + (12⁻¹ : ℂ) • (ramanujanE2_corr γ * ramanujanE2_corr γ) := by
  have hDslash := D_slash (k := (2 : ℤ)) (F := E₂) E₂_holo' γ
  ext z
  have hD := congrFun hDslash z
  have hE := congrFun (ramanujanE2_E₂_slash γ) z
  have hcorr_h := ramanujanE2_corr_holo γ
  let anom : ℂ :=
  ...
  have hDadd : D (E₂ ∣[(2 : ℤ)] γ) z = (D E₂ + D (ramanujanE2_corr γ)) z := by
    rw [ramanujanE2_E₂_slash]
    simp [D_add _ _ E₂_holo' hcorr_h]
  have hcorrD : D (ramanujanE2_corr γ) z =
      (-12⁻¹ : ℂ) * (ramanujanE2_corr γ z * ramanujanE2_corr γ z) := by
    simpa [Pi.mul_apply, Pi.neg_apply, mul_assoc] using congrFun (ramanujanE2_corr_D γ) z
  have hA :
      (2 : ℂ) * (2 * π * I)⁻¹ * (γ 1 0 / denom γ z) =
        (6⁻¹ : ℂ) * ramanujanE2_corr γ z := by
    simp [ramanujanE2_corr]; ring
  have hEeval : (E₂ ∣[(2 : ℤ)] γ) z = E₂ z + ramanujanE2_corr γ z := by
    simpa [Pi.add_apply] using hE
  have hanom : anom = (6⁻¹ : ℂ) * ramanujanE2_corr γ z * (E₂ z + ramanujanE2_corr γ z) := by
    dsimp [anom]
    rw [hEeval]
    have hA' := congrArg (fun t => t * (E₂ z + ramanujanE2_corr γ z)) hA
...
/-- `serre_D 1 E₂` is weight-4 slash-invariant under any `γ ∈ SL(2, ℤ)`. -/
private lemma ramanujanE2_serre_slash (γ : SL(2, ℤ)) :
    serre_D 1 E₂ ∣[(4 : ℤ)] γ = serre_D 1 E₂ := by
  ext z
  have hE := congrFun (ramanujanE2_E₂_slash γ) z
  have hD := congrFun (ramanujanE2_DE2_slash γ) z
  have hE_sq :
      (E₂ (γ • z)) ^ (2 : ℕ) * (denom γ z) ^ (-(4 : ℤ)) =
        (E₂ z + ramanujanE2_corr γ z) ^ (2 : ℕ) := by
    have hmain : E₂ (γ • z) * (denom γ z) ^ (-(2 : ℤ)) = E₂ z + ramanujanE2_corr γ z := by
  ...
  have hD' :
      D E₂ (γ • z) * (denom γ z) ^ (-(4 : ℤ)) =
        D E₂ z +
          (6⁻¹ : ℂ) * (E₂ z * ramanujanE2_corr γ z) +
          (12⁻¹ : ℂ) * (ramanujanE2_corr γ z * ramanujanE2_corr γ z) := by
    simpa [Pi.add_apply, Pi.mul_apply, Pi.smul_apply, smul_eq_mul] using hD
  grind only

/-- Package `serre_D 1 E₂` as a level-1 weight-4 modular form. -/
private noncomputable def ramanujanE2_F₄ : ModularForm Γ(1) 4 where
  toFun := serre_D 1 E₂
  slash_action_eq' := fun γ a =>
    slashaction_generators_GL2R (serre_D 1 E₂) 4 (ramanujanE2_serre_slash ModularGroup.S)
      (ramanujanE2_serre_slash ModularGroup.T) γ a
...
/-- The packaged form `ramanujanE2_F₄ z = serre_D 1 E₂ z` tends to `-(1/12)` at infinity. -/
private lemma ramanujanE2_F₄_tendsto :
  ...
  change Tendsto (fun z : ℍ => serre_D 1 E₂ z) atImInfty (𝓝 (-(12⁻¹ : ℂ)))
...
public theorem ramanujan_E₂' : serre_D 1 E₂ = - 12⁻¹ * E₄.toFun := by
  let G : ModularForm Γ(1) 4 := ramanujanE2_F₄ + (12⁻¹ : ℂ) • E₄
  have hGlim : Tendsto (fun z : ℍ => G z) atImInfty (𝓝 (0 : ℂ)) := by
    ...
    simpa [G, mul_one] using ramanujanE2_F₄_tendsto.add hE₄lim
  have hG0 : G = 0 := eq_zero_of_tendsto_zero_atImInfty (k := 4) (by norm_num) G hGlim
  funext z
  have hz : ramanujanE2_F₄ z + (12⁻¹ : ℂ) * E₄ z = 0 := by
    simpa [G] using congrArg (fun f : ModularForm Γ(1) 4 => f z) hG0
  have hz' : ramanujanE2_F₄ z = (-12⁻¹ : ℂ) * E₄ z := by
    simpa [neg_mul] using (eq_neg_iff_add_eq_zero).2 (by simpa using hz)
  assumption
