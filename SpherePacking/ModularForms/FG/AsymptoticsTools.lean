...
public lemma tendsto_ofComplex_I_mul_atTop_atImInfty :
  ...
  simpa [Function.comp_def] using (tendsto_id : Tendsto (fun t : ℝ => t) atTop atTop)

/-- The map `z ↦ cexp (c * z)` is holomorphic on the upper half-plane. -/
public lemma mdifferentiable_cexp_mul (c : ℂ) :
    MDifferentiable 𝓘(ℂ) 𝓘(ℂ) (fun z : UpperHalfPlane => cexp (c * (z : ℂ))) := fun z => by
...
public lemma ResToImagAxis.Real.D_of_real {F : UpperHalfPlane → ℂ}
  ...
  have hderivC : HasDerivAt F.resToImagAxis (deriv F.resToImagAxis t) t :=
    (ResToImagAxis.Differentiable F hFholo t ht).hasDerivAt
  ...
  have him_deriv : (deriv F.resToImagAxis t).im = 0 :=
    hderiv_im.unique ((hasDerivAt_const t (0 : ℝ)).congr_of_eventuallyEq h_im0)
