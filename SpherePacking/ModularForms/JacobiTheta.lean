...
@[grind =] private lemma I_mul_mul_I (x y : ℂ) : I * (x * (I * y)) = -(x * y) := by
  simp [mul_left_comm, mul_comm]

private lemma summable_Θ₂_term (τ : ℍ) : Summable (fun n : ℤ => Θ₂_term n τ) := by
...
private theorem Θ₂_imag_axis_real : ResToImagAxis.Real Θ₂ := by
  intro t ht
...
private lemma Θ₂_term_re_imag_axis (n : ℤ) (t : ℝ) (ht : 0 < t) :
...
private theorem Θ₂_imag_axis_pos : ResToImagAxis.Pos Θ₂ := by
...
public theorem H₂_imag_axis_pos : ResToImagAxis.Pos H₂ := by
  ...
  have hre : (H₂ τ).re = (Θ₂ τ).re ^ 4 := by
    have hpow : (Θ₂ τ) ^ 4 = ((Θ₂ τ).re : ℂ) ^ 4 := congrArg (· ^ 4) hΘeq
    change ((Θ₂ τ) ^ 4).re = (Θ₂ τ).re ^ 4
    rw [hpow, ← Complex.ofReal_pow, Complex.ofReal_re]
...
private lemma differentiableAt_jacobiTheta₂_half (τ : ℍ) :
...
private theorem isBoundedAtImInfty_H₃ : IsBoundedAtImInfty H₃ := by
  simp_rw [UpperHalfPlane.isBoundedAtImInfty_iff, H₃, Θ₃]
...
private noncomputable def jacobi_g : ℍ → ℂ := H₂ + H₄ - H₃

/-- The squared difference f := g² -/
private noncomputable def jacobi_f : ℍ → ℂ := jacobi_g ^ 2

/-- S-action on g: g|[2]S = -g -/
@[grind =]
private lemma jacobi_g_S_action : (jacobi_g ∣[(2 : ℤ)] S) = -jacobi_g := by
  ext z
...
@[grind =]
private lemma jacobi_g_T_action : (jacobi_g ∣[(2 : ℤ)] T) = -jacobi_g := by
  ext z
  simp [jacobi_g, sub_eq_add_neg, SlashAction.add_slash, SlashAction.neg_slash,
...
private lemma jacobi_g_mul_slash (γ : SL(2, ℤ)) :
  ...
  simpa [show (4 : ℤ) = 2 + 2 from rfl] using (mul_slash_SL2 2 2 γ jacobi_g jacobi_g)

/-- S-invariance of f: f|[4]S = f, because g|[2]S = -g. -/
private lemma jacobi_f_S_action : (jacobi_f ∣[(4 : ℤ)] S) = jacobi_f := by
  simpa [jacobi_f_eq_mul, jacobi_g_S_action] using (jacobi_g_mul_slash S)

/-- T-invariance of f: f|[4]T = f, because g|[2]T = -g. -/
private lemma jacobi_f_T_action : (jacobi_f ∣[(4 : ℤ)] T) = jacobi_f := by
...
private noncomputable def jacobi_f_SIF : SlashInvariantForm (CongruenceSubgroup.Gamma 1) 4 where
  toFun := jacobi_f
...
private lemma jacobi_g_MDifferentiable : MDifferentiable 𝓘(ℂ) 𝓘(ℂ) jacobi_g :=
...
private lemma jacobi_f_MDifferentiable : MDifferentiable 𝓘(ℂ) 𝓘(ℂ) jacobi_f := by
...
private theorem summable_rexp_neg_pi_int_sq : Summable (fun n : ℤ ↦ rexp (-π * n ^ 2)) := by
...
private theorem weighted_exp_sq_eventually_bound (w : ℤ → ℂ) (hw : ∀ n, ‖w n‖ ≤ 1) :
    ∀ᶠ z : ℍ in atImInfty, ∀ k : ℤ, ‖w k * cexp (π * I * k ^ 2 * z)‖ ≤ rexp (-π * k ^ 2) := by
...
private theorem weighted_exp_sq_term_tendsto_indicator (w : ℤ → ℂ) (hw0 : w 0 = 1) (k : ℤ) :
    Tendsto (fun z : ℍ ↦ w k * cexp (π * I * k ^ 2 * z)) atImInfty (𝓝 (if k = 0 then 1 else 0)) := by
  by_cases hk : k = 0
  · subst hk
...
private theorem tsum_weighted_exp_sq_tendsto_atImInfty (w : ℤ → ℂ) (hw0 : w 0 = 1)
    (hw : ∀ n, ‖w n‖ ≤ 1) :
...
private theorem jacobiTheta₂_zero_apply_tendsto_atImInfty :
...
private theorem jacobiTheta₂_half_apply_tendsto_atImInfty :
...
private noncomputable def jacobi_f_CF : CuspForm (Γ 1) 4 :=
  cuspFormOfSIFTendstoZero jacobi_f_SIF jacobi_f_MDifferentiable
    jacobi_f_tendsto_atImInfty

/-- jacobi_f = 0 by dimension argument: weight-4 cusp forms vanish. -/
private theorem jacobi_f_eq_zero : jacobi_f = 0 :=
  congr_arg (·.toFun)
...
private noncomputable def thetaDelta_f : ℍ → ℂ := H₂ * (H₃ * H₄)

private noncomputable def thetaDeltaFun : ℍ → ℂ := ((256 : ℂ)⁻¹) • (thetaDelta_f ^ 2)

private lemma thetaDeltaFun_div_exp_tendsto_atImInfty :
