...
/-!
# Polynomial Fourier Coefficient bounds (re-exports + application)

This file re-exports the `PolyFourierCoeffBound.*` sub-modules and specialises the bound
with the Fourier data of `(A_E)^2` to produce concrete exponential decay estimates on `φ₀`.
-/
