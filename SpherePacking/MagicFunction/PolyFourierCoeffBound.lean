...
private lemma aux_2 (x : ℂ) : 1 - Real.exp x.re ≤ norm (1 - cexp x) := calc
  norm (1 - cexp x) ≥ |norm (1 : ℂ) - norm (cexp x)| := abs_norm_sub_norm_le _ _
...
lemma aux_5 (z : ℍ) : norm (∏' (n : ℕ+), (1 - cexp (2 * ↑π * I * ↑↑n * z)) ^ 24) =
  ...
  simp only [← norm_pow]
  apply norm_tprod -- ℕ+ (fun n => (1 - cexp (2 * ↑π * I * n * z)) ^ 24)
...
private lemma step_8  :
  ...
  · exact aux_6 z
  · calc
    _ ≤ ∑' (n : ℕ), norm ((c (n + n₀)) * (cexp (↑π * I * ↑n * z))) :=
      norm_tsum_le_tsum_norm <| aux_3 z c n₀ hcsum
    _ = ∑' (n : ℕ), norm (c (n + n₀)) * norm (cexp (↑π * I * ↑n * z)) := by
      simp [norm_mul]
...
private lemma step_10 :
  ...
  · apply mul_nonneg (exp_nonneg (-π * (↑n₀ - 2) * z.im))
    apply tsum_nonneg
    intro i
    exact mul_nonneg (norm_nonneg _) (exp_nonneg _)
  · exact aux_8 z hz
...
private lemma step_12 :
  ...
  · -- This allows us to get rid of the numerators
    exact mul_nonneg (exp_nonneg _) <| tsum_nonneg <|
      fun _ ↦ mul_nonneg (norm_nonneg _) (exp_nonneg _)
  · exact aux_11
  · apply tprod_le_of_nonneg
    · intro n; simp
