...
/-!
# Polynomial Fourier Coefficient bounds: Fourier coefficients of `(A_E)^2` and application.

Computes the Fourier coefficients of `(A_E)^2` via the Cauchy product, repackages them as
`fouterm`, and specialises the polynomial Fourier-coefficient bound to produce concrete
exponential decay estimates on `φ₀`.
-/
