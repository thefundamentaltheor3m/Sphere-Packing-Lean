...
Defines kernels (`I₅.g`, `permI5Phase`, `permI5Kernel`) used to rewrite the Fourier transform
of `I₅` as an iterated integral, then bridges `intervalIntegral` definitions to `curveIntegral`
along straight segments, and defines the auxiliary Fourier-side integrand `Φ₁_fourier`.
-/
