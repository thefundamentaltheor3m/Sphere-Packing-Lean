...
lemma integrable_permI1Kernel_slice (w : ℝ⁸) (t : ℝ) (ht : t ∈ Ioc (0 : ℝ) 1) :
  ...
  have hg : Integrable g (volume : Measure ℝ⁸) := by
    ...
    have hc : Integrable
        (fun x : ℝ⁸ ↦ c * cexp ((π : ℂ) * I * (‖x‖ ^ 2 : ℝ) * z₁line t))
        (volume : Measure ℝ⁸) :=
      (integrable_gaussian_cexp_pi_mul_I_mul (z := z₁line t) hz).const_mul c
    have hg' : g = fun x : ℝ⁸ ↦ c * cexp ((π : ℂ) * I * (‖x‖ ^ 2 : ℝ) * z₁line t) := by
      funext x; simp [g, c, Φ₁', mul_assoc]
    simpa [hg'] using hc
...
lemma integral_norm_permI1Kernel_bound (w : ℝ⁸) (t : ℝ) (ht : t ∈ Ioc (0 : ℝ) 1) :
  ...
  have harg : (-1 / (z₁line t + 1) : ℂ) = (I : ℂ) / t := by
    have : (-1 / ((I : ℂ) * (t : ℂ)) : ℂ) = (I : ℂ) / t := by
      field_simp [ht_ne0]; simp [Complex.I_sq]
    simpa [z₁line_add_one] using this
  have hpow : ‖(z₁line t + 1) ^ 2‖ = t ^ 2 := by simp
  have hexp (x : ℝ⁸) :
...
lemma integrable_integral_norm_permI1Kernel (w : ℝ⁸) :
  ...
  have hz_half : (1 / 2 : ℝ) < z.im := by
    rw [hz_im]
    exact lt_of_lt_of_le (by norm_num) (one_le_inv_iff₀.2 ⟨ht0, ht1⟩)
  ...
  have := (integral_norm_permI1Kernel_bound (w := w) (t := t) ht).trans
