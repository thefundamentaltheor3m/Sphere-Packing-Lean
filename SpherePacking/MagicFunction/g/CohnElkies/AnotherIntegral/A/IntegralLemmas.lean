...
/-- The Laplace integrand `aLaplaceIntegrand u` is continuous on `(0, ∞)`. -/
private lemma continuousOn_aLaplaceIntegrand_Ioi (u : ℝ) :
    ContinuousOn (fun t : ℝ => aLaplaceIntegrand u t) (Set.Ioi (0 : ℝ)) := by
  have := continuousOn_phi0''_div_Ioi; unfold aLaplaceIntegrand; fun_prop

/-- Integrability of `aLaplaceIntegrand u` on `(0, 1]`: bounded by the Schwartz decay constant. -/
private lemma aLaplaceIntegrand_integrableOn_Ioc_zero_one {u : ℝ} (hu : 2 < u) :
    IntegrableOn (fun t : ℝ => aLaplaceIntegrand u t) (Set.Ioc (0 : ℝ) 1) := by
  ...
  refine MeasureTheory.IntegrableOn.of_bound (by simp : (MeasureTheory.volume : Measure ℝ)
    (Set.Ioc (0 : ℝ) 1) < ⊤)
    ((continuousOn_aLaplaceIntegrand_Ioi u).mono (fun _ ht => ht.1)
      |>.aestronglyMeasurable measurableSet_Ioc) C₀ ?_
...
`E₂, E₄, E₆, Δ⁻¹` apply. -/
private lemma aLaplaceIntegrand_integrableOn_Ioi_one {u : ℝ} (hu : 2 < u) :
    IntegrableOn (fun t : ℝ => aLaplaceIntegrand u t) (Set.Ioi (1 : ℝ)) := by
...
public lemma aLaplaceIntegral_convergent {u : ℝ} (hu : 2 < u) :
  ...
  rw [← Set.Ioc_union_Ioi_eq_Ioi (a := (0 : ℝ)) (b := 1) zero_le_one]
  exact (aLaplaceIntegrand_integrableOn_Ioc_zero_one hu).union
    (aLaplaceIntegrand_integrableOn_Ioi_one hu)
...
/-- Pointwise bound `‖base t * exp(u * k t)‖ ≤ Cbase * exp(‖u‖ * K)` from norm bounds on
`base` and `k`. -/
private lemma norm_base_mul_exp_le {base k : ℝ → ℂ} {Cbase K : ℝ} {t : ℝ}
    (ht : t ∈ Ι (0 : ℝ) 1)
    (hbase_bound : ∀ t ∈ Ι (0 : ℝ) 1, ‖base t‖ ≤ Cbase)
    (hk_bound : ∀ t ∈ Ι (0 : ℝ) 1, ‖k t‖ ≤ K) (u : ℂ) :
    ‖base t * Complex.exp (u * k t)‖ ≤ Cbase * Real.exp (‖u‖ * K) :=
  norm_mul_le_of_le (hbase_bound t ht) <| (Complex.norm_exp_le_exp_norm _).trans
    (Real.exp_le_exp.2 ((norm_mul_le u (k t)).trans (by gcongr; exact hk_bound t ht)))

/-- Pointwise derivative bound `‖base t * k t * exp(u * k t)‖ ≤ Cbase * (K * E)` for `u` in a
ball of radius `1` around `u0`. -/
private lemma norm_base_k_mul_exp_le {base k : ℝ → ℂ} {Cbase K : ℝ} {u0 u : ℂ} {t : ℝ}
    (ht : t ∈ Ι (0 : ℝ) 1) (hu : u ∈ Metric.ball u0 (1 : ℝ))
    (hbase_bound : ∀ t ∈ Ι (0 : ℝ) 1, ‖base t‖ ≤ Cbase)
    (hk_bound : ∀ t ∈ Ι (0 : ℝ) 1, ‖k t‖ ≤ K) :
    ‖base t * k t * Complex.exp (u * k t)‖ ≤ Cbase * (K * Real.exp ((‖u0‖ + 1) * K)) := by
  have hCbase : 0 ≤ Cbase := (norm_nonneg _).trans (hbase_bound t ht)
  have hK : 0 ≤ K := (norm_nonneg _).trans (hk_bound t ht)
  have hu' : ‖u‖ ≤ ‖u0‖ + 1 := by
    have h1 : ‖u‖ ≤ ‖u0‖ + ‖u - u0‖ := by
  ...
  calc ‖base t * k t * Complex.exp (u * k t)‖
      = ‖base t‖ * (‖k t‖ * ‖Complex.exp (u * k t)‖) := by
        simp [mul_left_comm, mul_comm]
    _ ≤ ‖base t‖ * (‖k t‖ * Real.exp ((‖u0‖ + 1) * K)) := by
        gcongr
        exact (Complex.norm_exp_le_exp_norm _).trans
          (Real.exp_le_exp.2 (norm_mul_le_of_le hu' (hk_bound t ht)))
    _ ≤ Cbase * (K * Real.exp ((‖u0‖ + 1) * K)) := by
        gcongr <;> [exact hbase_bound t ht; exact hk_bound t ht]
...
public lemma differentiableAt_intervalIntegral_mul_exp
  ...
  have hF_meas : ∀ᶠ u in 𝓝 u0, AEStronglyMeasurable (F u) (volume.restrict (Ι (0 : ℝ) 1)) :=
    .of_forall fun u => (hbase.mul (hexp u)).aestronglyMeasurable
  ...
  have hF'_meas : AEStronglyMeasurable (F' u0) (volume.restrict (Ι (0 : ℝ) 1)) := by
    simpa [F', mul_assoc] using (hbase.mul (hk.mul (hexp u0))).aestronglyMeasurable
      (μ := (volume : Measure ℝ)) measurableSet_uIoc
  have hF_int : IntervalIntegrable (F u0) volume (0 : ℝ) 1 := intervalIntegrable_iff.2 <|
    MeasureTheory.IntegrableOn.of_bound (by simp : (volume : Measure ℝ) (Ι (0 : ℝ) 1) < ⊤)
      hF_meas.self_of_nhds (Cbase * Real.exp (‖u0‖ * K))
      ((ae_restrict_iff' measurableSet_uIoc).2 <| .of_forall fun t ht =>
        norm_base_mul_exp_le ht hbase_bound hk_bound u0)
  exact (intervalIntegral.hasDerivAt_integral_of_dominated_loc_of_deriv_le
    (μ := (volume : Measure ℝ)) (F := F) (F' := F') (s := Metric.ball u0 (1 : ℝ))
    (bound := fun _ => Cbase * (K * Real.exp ((‖u0‖ + 1) * K)))
    (Metric.ball_mem_nhds u0 (by norm_num)) hF_meas hF_int hF'_meas
    (.of_forall fun t ht u hu => norm_base_k_mul_exp_le ht hu hbase_bound hk_bound) (by simp)
    (.of_forall fun t _ u _ => by
      simpa [F, F', mul_assoc, mul_left_comm, mul_comm] using
        ((Complex.hasDerivAt_exp (u * k t)).comp u
          (hasDerivAt_mul_const (k t) (x := u))).const_mul (base t))).2.differentiableAt
...
lemma I₆'C_differentiableAt (u0 : ℂ) (hu0 : u0 ∈ rightHalfPlane) :
  ...
  have hI₆'C_eq : I₆'C = fun u : ℂ =>
      2 * ∫ t in Set.Ici (1 : ℝ), base₆ t * Complex.exp (-(π : ℂ) * u * (t : ℂ)) := by
    funext u; simp [I₆'C, base₆, mul_assoc, mul_left_comm, mul_comm]
  exact hI₆'C_eq ▸ (differentiableAt_integral_Ici_one_base_exp
    base₆_continuousOn hbase_bound hu0re).const_mul (2 : ℂ)
...
/-- Summability of `(n+2)^5 * r^n` for `0 ≤ r < 1`, with values in `ℝ`. -/
private lemma summable_pow5_shift_mul_geom {r : ℝ} (hr_nonneg : 0 ≤ r) (hr_lt_one : r < 1) :
    Summable (fun n : ℕ => ((n + 2 : ℝ) ^ 5) * r ^ n) := by
  refine Summable.of_nonneg_of_le
    (f := fun n : ℕ => (512 : ℝ) * ((((n : ℝ) ^ 5 + 1) : ℝ) * r ^ n))
    (g := fun n => ((n + 2 : ℝ) ^ 5) * r ^ n)
    (fun n => by positivity) (fun n => ?_) <| Summable.mul_left (512 : ℝ) <| by
      simpa [show (fun n : ℕ => (((n : ℝ) ^ 5 + 1) : ℝ) * r ^ n) =
          (fun n : ℕ => ((n : ℝ) ^ 5 : ℝ) * r ^ n) + fun n : ℕ => r ^ n by
        funext n; simp [add_mul]] using
        (summable_pow_mul_geometric_of_norm_lt_one (R := ℝ) 5
          (by simpa [Real.norm_of_nonneg hr_nonneg] using hr_lt_one)).add
  ...
  nlinarith [hbase, pow_nonneg (by positivity : (0 : ℝ) ≤ n) 5, pow_nonneg hr_nonneg n]

/-- Rewrite `(∑' n ≥ 1, n · σ₃(n) · q^n) - q` as the index-shifted series starting at `n+2`. -/
private lemma tsum_pos_sub_first_eq_shift_two {z : ℍ} {q q1 : ℝ}
    (hq_nonneg : 0 ≤ q) (hq_le : q ≤ q1)
    (hqC : Periodic.qParam (1 : ℝ) z = (q : ℂ))
    (hb_summ : Summable (fun n : ℕ => ((n + 2 : ℝ) ^ 5) * q1 ^ n)) :
    (∑' n : ℕ+, n * (σ 3 n) * cexp (2 * π * Complex.I * n * z)) -
        cexp (2 * π * Complex.I * z) =
      ∑' n : ℕ, ((n + 2 : ℂ) * (σ 3 (n + 2) : ℂ)) *
        cexp (2 * π * Complex.I * (n + 2 : ℂ) * z) := by
  set g : ℕ → ℂ := fun n => (n + 1) * (σ 3 (n + 1)) * cexp (2 * π * Complex.I * (n + 1) * z)
  have hg_summ : Summable g := .of_norm_bounded (hb_summ.mul_left q) fun n => by
    rw [show ‖g n‖ = ‖((n + 1 : ℂ) * (σ 3 (n + 1) : ℂ))‖ *
      ...
      show q * ((n + 2 : ℝ) ^ 5 * q1 ^ n) = ((n + 2 : ℝ) ^ 5 : ℝ) * (q * q1 ^ n) by ring]
    gcongr
...
/-- Norm bound `‖(n+2)·σ₃(n+2)·q^(n+2)‖ ≤ q² · (n+2)⁵ · q₁ⁿ` for the shifted Eisenstein series. -/
private lemma norm_shift_two_term_le {z : ℍ} {q q1 : ℝ} (hq_nonneg : 0 ≤ q) (hq_le : q ≤ q1)
    (hqC : Periodic.qParam (1 : ℝ) z = (q : ℂ)) (n : ℕ) :
    ‖((n + 2 : ℂ) * (σ 3 (n + 2) : ℂ)) * cexp (2 * π * Complex.I * (n + 2 : ℂ) * z)‖ ≤
      q ^ (2 : ℕ) * (((n + 2 : ℝ) ^ 5) * q1 ^ n) := by
  rw [show ‖((n + 2 : ℂ) * (σ 3 (n + 2) : ℂ)) * cexp (2 * π * Complex.I * (n + 2 : ℂ) * z)‖ =
      ‖((n + 2 : ℂ) * (σ 3 (n + 2) : ℂ))‖ *
        ‖cexp (2 * π * Complex.I * (n + 2 : ℂ) * z)‖ by simp [mul_assoc],
    show q ^ (2 : ℕ) * (((n + 2 : ℝ) ^ 5) * q1 ^ n) =
      ((n + 2 : ℝ) ^ 5 : ℝ) * (q ^ (2 : ℕ) * q1 ^ n) by ring]
  gcongr
...
/-- Core norm bound for `E₂·E₄ - E₆ - 720q` at a fixed point `z`. -/
private lemma norm_E2E4_sub_E6_sub_720q_le {z : ℍ} {q q1 : ℝ}
    (hq_nonneg : 0 ≤ q) (hq_le : q ≤ q1)
    (hqC : Periodic.qParam (1 : ℝ) z = (q : ℂ))
    (hb_summ : Summable (fun n : ℕ => ((n + 2 : ℝ) ^ 5) * q1 ^ n)) :
    ‖(E₂ z) * (E₄ z) - (E₆ z) - (720 : ℂ) * (q : ℂ)‖ ≤
      (720 : ℝ) * (q ^ (2 : ℕ)) * (∑' n : ℕ, ((n + 2 : ℝ) ^ 5) * q1 ^ n) := by
  set b : ℕ → ℝ := fun n => ((n + 2 : ℝ) ^ 5) * q1 ^ n
  set f : ℕ → ℂ := fun n =>
    ((n + 2 : ℂ) * (σ 3 (n + 2) : ℂ)) * cexp (2 * π * Complex.I * (n + 2 : ℂ) * z)
  have hf_le : ∀ n, ‖f n‖ ≤ q ^ (2 : ℕ) * b n :=
    fun n => norm_shift_two_term_le hq_nonneg hq_le hqC n
  ...
  rw [show (E₂ z) * (E₄ z) - (E₆ z) - (720 : ℂ) * (q : ℂ) = (720 : ℂ) * (∑' n : ℕ, f n) by
      rw [E₂_mul_E₄_sub_E₆ z, ← show
        (∑' n : ℕ+, n * (σ 3 n) * cexp (2 * π * Complex.I * n * z)) - (q : ℂ) = ∑' n : ℕ, f n by
          simpa [show cexp (2 * π * Complex.I * z) = (q : ℂ) from by
            simpa [Periodic.qParam] using hqC] using
              tsum_pos_sub_first_eq_shift_two hq_nonneg hq_le hqC hb_summ]
      ring, norm_mul, show ‖(720 : ℂ)‖ = 720 by simp, mul_assoc]
...
public lemma exists_E2E4_sub_E6_sub_720q_bound :
  ...
  set q1 : ℝ := Real.exp (-2 * π)
  set b : ℕ → ℝ := fun n => ((n + 2 : ℝ) ^ 5) * q1 ^ n
  have hb_summ : Summable b :=
    summable_pow5_shift_mul_geom (Real.exp_pos _).le exp_neg_two_pi_lt_one
  refine ⟨1 + (720 : ℝ) * (∑' n : ℕ, b n), by positivity, fun t ht0 ht1 => ?_⟩
  set z : ℍ := zI t ht0; set q : ℝ := Real.exp (-2 * π * t)
  have hq_nonneg : 0 ≤ q := (Real.exp_pos _).le
  have hmain := norm_E2E4_sub_E6_sub_720q_le (z := z) (q := q) (q1 := q1) hq_nonneg
    (q_le_q1 (t := t) ht1) (by simpa [q, z] using qParam_zI t ht0) hb_summ
...
/-- From `‖E₄ z - (1 + 240 q)‖ ≤ CE4 · q²` derive the quadratic bound for `E₄² - (1 + 480 q)`. -/
private lemma norm_E4_sq_sub_one_sub_480q_le {z : ℍ} {q q1 CE4 B240 : ℝ}
    (hq_nonneg : 0 ≤ q) (hq_le_q1 : q ≤ q1) (hq_le_one : q ≤ 1)
    (hE4err : ‖E₄ z - ((1 : ℂ) + (240 : ℂ) * (q : ℂ))‖ ≤ CE4 * q ^ (2 : ℕ))
    (hB240 : B240 = 1 + 240 * q1) :
  ...
  have hbase_norm : ‖((1 : ℂ) + (240 : ℂ) * (q : ℂ))‖ ≤ B240 := by
    ...
    simp [abs_of_nonneg hq_nonneg, hB240] at this ⊢; linarith [hq_le_q1]
  simpa [hE.symm] using norm_base_add_e_sq_sub_one_sub_480q_le (q := q) (CE4 := CE4)
    (B240 := B240) hq_nonneg hq_le_one he hbase_norm

/-- From the third-order Δ bound, derive that `exp(2π t)·Δ z` is `1 + (-24)q + O(q²)`. -/
private lemma norm_exp_mul_Delta_sub_one_sub_neg24q_le {z : ℍ} {t q CΔ3 : ℝ}
    (hΔ3err : ‖Δ z - ((q : ℂ) + (-24 : ℂ) * ((q : ℂ) ^ (2 : ℕ)))‖ ≤ CΔ3 * q ^ (3 : ℕ))
    (hExpq : (Real.exp (2 * π * t)) * q = 1) :
...
/-- Final bound: combine `‖Δ⁻¹‖`, the numerator bound, and `q² · exp(2π t) = q`. -/
private lemma phi4_le_K_mul_q {z : ℍ} {t q K CΔinv : ℝ}
    (hK_nonneg : 0 ≤ K)
    (hΔinv' : ‖(Δ z)⁻¹‖ ≤ CΔinv * Real.exp (2 * π * t))
    (hnum : ‖(E₄ z) ^ (2 : ℕ) - (Real.exp (2 * π * t) : ℂ) * (Δ z) - (504 : ℂ) * (Δ z)‖ ≤
        K * q ^ (2 : ℕ))
    (hq2 : q ^ (2 : ℕ) * Real.exp (2 * π * t) = q) :
    ‖φ₄' z - (Real.exp (2 * π * t) : ℂ) - (504 : ℂ)‖ ≤ (CΔinv * K) * q := by
  calc ‖φ₄' z - (Real.exp (2 * π * t) : ℂ) - (504 : ℂ)‖
        = ‖(E₄ z)^2 - (Real.exp (2*π*t) : ℂ) * Δ z - (504:ℂ) * Δ z‖ * ‖(Δ z)⁻¹‖ := by
          rw [show φ₄' z - (Real.exp (2 * π * t) : ℂ) - (504 : ℂ) =
            ((E₄ z) ^ (2 : ℕ) - (Real.exp (2 * π * t) : ℂ) * (Δ z) -
              (504 : ℂ) * (Δ z)) / (Δ z) from by
            dsimp [φ₄']; field_simp [Δ_ne_zero z], norm_div, div_eq_mul_inv, norm_inv]
    _ ≤ (K * q ^ (2 : ℕ)) * (CΔinv * Real.exp (2 * π * t)) :=
        mul_le_mul hnum hΔinv' (norm_nonneg _) (mul_nonneg hK_nonneg (by positivity))
    _ = (CΔinv * K) * q := by linear_combination (CΔinv * K) * hq2

/-- Assemble the numerator bound for `φ₄'` from the q-expansion remainders for `E₄`, `Δ`. -/
private lemma phi4_numerator_assemble {z : ℍ} {t q q1 CE4 CΔq CΔ3 : ℝ}
    (hq_nonneg : 0 ≤ q) (hq_le_q1 : q ≤ q1) (hq_le_one : q ≤ 1)
    (hExpq : Real.exp (2 * π * t) * q = 1)
    (hE4err : ‖E₄ z - ((1 : ℂ) + (240 : ℂ) * (q : ℂ))‖ ≤ CE4 * q ^ (2 : ℕ))
    (hΔq : ‖Δ z - (q : ℂ)‖ ≤ CΔq * q ^ (2 : ℕ))
    (hΔ3 : ‖Δ z - ((q : ℂ) + (-24 : ℂ) * ((q : ℂ) ^ (2 : ℕ)))‖ ≤ CΔ3 * q ^ (3 : ℕ)) :
    ‖(E₄ z) ^ (2 : ℕ) - (Real.exp (2 * π * t) : ℂ) * (Δ z) - (504 : ℂ) * (Δ z)‖ ≤
      ((240 ^ 2 : ℝ) + 2 * (1 + 240 * q1) * CE4 + CE4 ^ 2 + CΔ3 + 504 * CΔq) *
        q ^ (2 : ℕ) :=
  phi4_numerator_bound
    (norm_E4_sq_sub_one_sub_480q_le (z := z) hq_nonneg hq_le_q1 hq_le_one hE4err rfl)
    (norm_exp_mul_Delta_sub_one_sub_neg24q_le (z := z) (t := t) hΔ3 hExpq) hΔq
...
public lemma exists_phi4'_sub_exp_sub_504_bound_ge :
  ...
  rcases exists_E4_sub_one_sub_240q_bound with ⟨CE4, _, hE4⟩
  rcases exists_Delta_sub_q_bound with ⟨CΔq, _, hΔq⟩
  rcases exists_Delta_sub_q_sub_neg24_qsq_bound with ⟨CΔ3, _, hΔ3⟩
  set K : ℝ := (240 ^ 2 : ℝ) + 2 * (1 + 240 * Real.exp (-2 * π)) * CE4 +
    CE4 ^ 2 + CΔ3 + 504 * CΔq
  refine ⟨1 + CΔinv * K, max 1 AΔ, by positivity, le_max_left _ _, fun t ht0 htA => ?_⟩
  have ht1 : 1 ≤ t := le_trans (le_max_left _ _) htA
  refine (phi4_le_K_mul_q (K := K) (by dsimp [K]; positivity)
    (show ‖(Δ (zI t ht0))⁻¹‖ ≤ CΔinv * Real.exp (2 * π * t) by
      simpa [zI_im t ht0] using hΔinv (zI t ht0)
        (by simpa [zI_im t ht0] using (le_max_right _ _ : AΔ ≤ _).trans htA))
    (phi4_numerator_assemble (q1 := Real.exp (-2 * π)) (Real.exp_pos _).le (q_le_q1 ht1)
      ((q_le_q1 ht1).trans exp_neg_two_pi_lt_one.le) (by rw [← Real.exp_add]; simp)
      (hE4 t ht0 ht1) (hΔq t ht0 ht1) (by simpa [pow_two] using hΔ3 t ht0 ht1))
    (by rw [← Real.exp_nat_mul, ← Real.exp_add]; ring_nf)).trans
...
/-- Combine the three component bounds on `φ₀`, `φ₂' - 720`, `φ₄' - exp - 504` into
the `Clarge` bound on `‖cancelExpr t‖`, valid for `t ≥ 1`. -/
private lemma cancelExpr_norm_le_of_phi_bounds {t : ℝ} (ht0 : 0 < t) (ht1 : 1 ≤ t)
    {C₀ C₂ C₄ : ℝ} (hC₂pos : 0 < C₂) (hC₄pos : 0 < C₄)
    (hφ₀z : ‖φ₀'' ((Complex.I : ℂ) * (t : ℂ))‖ ≤ C₀ * Real.exp (-2 * π * t))
    (hφ₂z : ‖φ₂' (zI t ht0) - (720 : ℂ)‖ ≤ C₂ * Real.exp (-2 * π * t))
    (hφ₄z : ‖φ₄' (zI t ht0) - (Real.exp (2 * π * t) : ℂ) - (504 : ℂ)‖ ≤
        C₄ * Real.exp (-2 * π * t)) :
    ‖cancelExpr t‖ ≤
      (C₀ + (12 / π) * C₂ + (36 / (π ^ (2 : ℕ))) * C₄) * (t ^ (2 : ℕ)) *
        Real.exp (-2 * π * t) := by
  set z : ℍ := zI t ht0 with hz
  ...
  have hφ₀z' : ‖φ₀ z‖ ≤ C₀ * Real.exp (-2 * π * t) := by
    simpa [show φ₀'' ((Complex.I : ℂ) * (t : ℂ)) = φ₀ z by
      simpa [z, zI] using (φ₀''_coe_upperHalfPlane z)] using hφ₀z
  have hnorm1 : ‖((t ^ (2 : ℕ) : ℝ) : ℂ) * φ₀ z‖ ≤
    ...
    nlinarith [hφ₀z', norm_nonneg (φ₀ z), hexp0]
  have hnorm2 :
  ...
  nlinarith [hexp0, sq_nonneg t]

/-- Compactness: `‖cancelExpr‖` is bounded on every compact interval `[1, A]`. -/
private lemma exists_cancelExpr_bound_on_Icc {A : ℝ} (hA : 1 ≤ A) :
    ∃ M : ℝ, ∀ t : ℝ, 1 ≤ t → t ≤ A → ‖cancelExpr t‖ ≤ M := by
...
/-- Scaling: for `1 ≤ t < A` and `0 ≤ M`, the constant `M` is bounded by
`(M / exp(-2πA)) * (t² * exp(-2πt))`. -/
private lemma exp_decay_scale_lt {A t M : ℝ} (ht1 : 1 ≤ t) (htA : t < A) (hM : 0 ≤ M) :
  ...
  simpa [show (M / Real.exp (-2 * π * A)) * Real.exp (-2 * π * A) = M by
    ...
    ((Real.exp_le_exp.2 <| mul_le_mul_of_nonpos_left htA.le
    ...
    (div_nonneg hM (Real.exp_pos (-2 * π * A)).le)

lemma exists_phi0_cancellation_bound :
    ∃ C : ℝ, ∀ t : ℝ, 1 ≤ t →
        ‖cancelExpr t‖ ≤ C * (t ^ (2 : ℕ)) * Real.exp (-2 * π * t) := by
  rcases norm_φ₀_imag_le with ⟨C₀, _, hφ₀⟩
  rcases exists_phi2'_sub_720_bound_ge with ⟨C₂, A₂, hC₂pos, hA₂, hφ₂⟩
  rcases exists_phi4'_sub_exp_sub_504_bound_ge with ⟨C₄, A₄, hC₄pos, _, hφ₄⟩
  set A : ℝ := max A₂ A₄
  set Clarge : ℝ := C₀ + (12 / π) * C₂ + (36 / (π ^ (2 : ℕ))) * C₄
  obtain ⟨M, hM⟩ := exists_cancelExpr_bound_on_Icc (hA₂.trans (le_max_left _ _) : 1 ≤ A)
  refine ⟨max Clarge (M / Real.exp (-2 * π * A)), fun t ht1 => ?_⟩
  have ht0 : 0 < t := zero_lt_one.trans_le ht1
  by_cases htA : A ≤ t
  · exact (cancelExpr_norm_le_of_phi_bounds ht0 ht1 hC₂pos hC₄pos (hφ₀ t ht1)
      (hφ₂ t ht0 ((le_max_left _ _).trans htA))
      (hφ₄ t ht0 ((le_max_right _ _).trans htA))).trans (by gcongr; exact le_max_left _ _)
  · have hbound := hM t ht1 (le_of_not_ge htA)
    nlinarith [hbound, exp_decay_scale_lt (A := A) (M := M) ht1 (lt_of_not_ge htA)
      ((norm_nonneg _).trans hbound), mul_le_mul_of_nonneg_right
      (le_max_right Clarge (M / Real.exp (-2 * π * A)))
...
/-- A `Cφ₀`-bound on `‖φ₀''(i/t)‖` for `t ∈ (0, 1]`, derived from the Fourier-coefficient bound. -/
private lemma norm_phi0''_Idiv_le_on_Ioc {t : ℝ} (ht : t ∈ Set.Ioc (0 : ℝ) 1)
    {Cφ₀ : ℝ} (hCφ₀_pos : 0 < Cφ₀)
    (hCφ₀ : ∀ z : ℍ, (1 / 2 : ℝ) < z.im → ‖φ₀ z‖ ≤ Cφ₀ * Real.exp (-2 * π * z.im)) :
...
/-- Term-by-term bound on `‖cancelExpr t‖` for `t ∈ (0, 1]`. -/
private lemma norm_cancelExpr_le_on_Ioc {t : ℝ} (ht : t ∈ Set.Ioc (0 : ℝ) 1)
    {Cφ₀ : ℝ} (hφ0'' : ‖φ₀'' ((Complex.I : ℂ) / (t : ℂ))‖ ≤ Cφ₀) :
    ‖cancelExpr t‖ ≤ (t ^ (2 : ℕ) : ℝ) * Cφ₀ +
...
lemma aAnotherIntegrand_integrableOn_Ioc {u : ℝ} (hu : 0 < u) :
  ...
  have hφ0'' := norm_phi0''_Idiv_le_on_Ioc ht hCφ₀_pos hCφ₀
  ...
  nlinarith [hmul.trans (norm_cancelExpr_le_on_Ioc ht hφ0''),
    mul_le_mul_of_nonneg_right ht2_le hCφ₀_pos.le]
...
/-- Split the imaginary axis integral of `Φ₅'` over `Ioi 0` into `Ioc 0 1` and `Ioi 1` pieces. -/
private lemma integral_Φ₅'_Ioc_add_Ioi {u : ℝ} (hu : 2 < u) :
  ...
  simpa [add_comm, add_left_comm, add_assoc] using
    (MeasureTheory.setIntegral_union (μ := volume)
      ...
      (ht := measurableSet_Ioi)
      (h5Ioi0.mono_set (fun t (ht : t ∈ Set.Ioc (0 : ℝ) 1) => ht.1))
      (integrableOn_Φ₅'_imag_axis (u := u) hu)).symm

/-- The trig coefficient `e^{iπu} + e^{-iπu} - 2 = -4 sin²(πu/2)`. -/
private lemma exp_add_exp_sub_two_eq_neg_four_sin_sq (u : ℝ) :
    (Complex.exp (((π * u : ℝ) : ℂ) * Complex.I) +
        Complex.exp (-(((π * u : ℝ) : ℂ) * Complex.I)) - (2 : ℂ)) =
      -((4 * (Real.sin (π * u / 2)) ^ (2 : ℕ) : ℝ) : ℂ) := by
  have hrew : (Complex.exp (((π * u : ℝ) : ℂ) * Complex.I) +
      Complex.exp (-(((π * u : ℝ) : ℂ) * Complex.I)) - (2 : ℂ)) =
      -((2 : ℂ) - Complex.exp (((π * u : ℝ) : ℂ) * Complex.I) -
        Complex.exp (-(((π * u : ℝ) : ℂ) * Complex.I))) := by ring
...
/-- The Ioi-integral of `Φ₅'(t I)` equals minus the Laplace integral of `aLaplaceIntegrand`. -/
private lemma integral_Φ₅'_eq_neg_aLaplace (u : ℝ) :
...
public theorem aRadial_eq_laplace_phi0_main {u : ℝ} (hu : 2 < u) :
  ...
  rw [show MagicFunction.a.RealIntegrals.I₁' u + MagicFunction.a.RealIntegrals.I₂' u +
        MagicFunction.a.RealIntegrals.I₃' u + MagicFunction.a.RealIntegrals.I₄' u +
        MagicFunction.a.RealIntegrals.I₅' u + MagicFunction.a.RealIntegrals.I₆' u =
      (MagicFunction.a.RealIntegrals.I₁' u + MagicFunction.a.RealIntegrals.I₃' u +
        MagicFunction.a.RealIntegrals.I₅' u) +
      (MagicFunction.a.RealIntegrals.I₂' u + MagicFunction.a.RealIntegrals.I₄' u +
        MagicFunction.a.RealIntegrals.I₆' u) from by ring,
    I₁'_add_I₃'_add_I₅'_eq_imag_axis (u := u),
    I₂'_add_I₄'_add_I₆'_eq_imag_axis_tail (u := u) hu,
    intervalIntegral.integral_of_le (zero_le_one : (0 : ℝ) ≤ 1),
    show ∀ A B C : ℂ,
      (Complex.I : ℂ) * (C * A) + (Complex.I : ℂ) * (C * B) = (Complex.I : ℂ) * (C * (A + B))
      from fun _ _ _ ↦ by ring,
    integral_Φ₅'_Ioc_add_Ioi hu, exp_add_exp_sub_two_eq_neg_four_sin_sq u,
    integral_Φ₅'_eq_neg_aLaplace u]
...
/-- A homeomorphism `(Ioi 0) × ℝ ≃ₜ {u : ℂ // 0 < u.re}` sending `(x, y)` to `x + yi`. -/
private noncomputable def rightHalfPlaneProdHomeomorph :
    (Set.Ioi (0 : ℝ) × ℝ) ≃ₜ {u : ℂ // 0 < u.re} where
  toEquiv :=
    { toFun := fun p =>
        ⟨(p.1 : ℝ) + p.2 * Complex.I, by have hp : (0 : ℝ) < (p.1 : ℝ) := p.1.2; simpa using hp⟩
      invFun := fun z => (⟨z.1.re, z.2⟩, z.1.im)
      left_inv := by rintro ⟨x, y⟩; ext <;> simp
      right_inv := by rintro ⟨z, hz⟩; exact Subtype.ext (by simp [Complex.re_add_im]) }
  continuous_toFun :=
    (show Continuous (fun p : Set.Ioi (0 : ℝ) × ℝ => (p.1 : ℝ) + p.2 * Complex.I) by
      fun_prop).subtype_mk fun p => by have hp : (0 : ℝ) < (p.1 : ℝ) := p.1.2; simpa using hp
  continuous_invFun := by fun_prop

/-- A homeomorphism `ℂ ≃ₜ {u : ℂ // 0 < u.re}` obtained by exponentiating the real part. -/
private noncomputable def rightHalfPlaneHomeomorph : ℂ ≃ₜ {u : ℂ // 0 < u.re} :=
  (Complex.equivRealProdCLM.toHomeomorph.trans
    ((Real.expOrderIso.toHomeomorph).prodCongr (Homeomorph.refl ℝ))).trans
    rightHalfPlaneProdHomeomorph
...
public lemma ACDomain_isPreconnected : IsPreconnected ACDomain := by
  ...
  let h := rightHalfPlaneHomeomorph
  have hpre : IsPreconnected ({z2}ᶜ : Set {u : ℂ // 0 < u.re}) := by
    ...
    have himage : h '' ({h.symm z2}ᶜ : Set ℂ) = ({z2}ᶜ : Set {u : ℂ // 0 < u.re}) := by
      ext z
  ...
  have hval : ((Subtype.val : {u : ℂ // 0 < u.re} → ℂ) '' ({z2}ᶜ :
    ...
    · rintro hu
      refine ⟨⟨u, by simpa [rightHalfPlane] using hu.1⟩, ?_, rfl⟩
...
/-- Trading exponential decay: `(π t) e^(-πεt) ≤ (2/ε) e^(-πε t/2)` for `0 < ε`, using
`x ≤ e^x` to absorb the factor of `t`. -/
private lemma pi_t_exp_neg_pi_eps_le {ε : ℝ} (hε : 0 < ε) (t : ℝ) :
  ...
  have := mul_le_mul_of_nonneg_left hbase Real.pi_pos.le
  have heq : π * ((2 / (π * ε)) * Real.exp (-(π * ε / 2) * t)) =
      (2 / ε) * Real.exp (-π * (ε / 2) * t) := by
    rw [show (-(π * ε / 2) * t) = (-π * (ε / 2) * t) from by ring]
    field_simp
  calc (π * t) * Real.exp (-π * ε * t)
      = π * (t * Real.exp (-(π * ε) * t)) := by
        rw [show (-(π * ε) * t) = (-π * ε * t) from by ring]; ring
    _ ≤ π * ((2 / (π * ε)) * Real.exp (-(π * ε / 2) * t)) := this
    _ = (2 / ε) * Real.exp (-π * (ε / 2) * t) := heq

/-- Dominating bound for `‖(-π t) * (base t * exp(-π z t))‖` on `Metric.ball u ε` when
`ε ≤ z.re`, in terms of the `ε/2`-scaled integrand norm. -/
private lemma norm_deriv_integrand_le {base : ℝ → ℂ} {u z : ℂ} {ε t : ℝ}
    (hε : 0 < ε) (htpos : 0 < t) (hu : u.re = 2 * ε)
    (hz : z ∈ Metric.ball u ε) :
    ‖(-(π : ℂ) * (t : ℂ)) * (base t * Complex.exp (-(π : ℂ) * z * (t : ℂ)))‖ ≤
      (2 / ε) * ‖base t * (Real.exp (-π * (ε / 2) * t) : ℂ)‖ := by
  have hzre : ε ≤ z.re := by
    have hlt : |z.re - u.re| < ε := lt_of_le_of_lt (abs_re_le_norm (z - u))
      (by simpa [Metric.mem_ball, dist_eq_norm] using hz : ‖z - u‖ < ε)
    nlinarith [(abs_lt.mp hlt).1, hu]
  have hnorm_integ : ‖base t * Complex.exp (-(π : ℂ) * z * (t : ℂ))‖ ≤
      ‖base t‖ * Real.exp (-π * ε * t) := by
    rw [norm_mul]
    exact mul_le_mul_of_nonneg_left (norm_exp_le_of_re_ge htpos.le hzre) (norm_nonneg _)
  ...
  calc ‖(-(π : ℂ) * (t : ℂ)) * (base t * Complex.exp (-(π : ℂ) * z * (t : ℂ)))‖
      = ‖(-(π : ℂ) * (t : ℂ))‖ * ‖base t * Complex.exp (-(π : ℂ) * z * (t : ℂ))‖ := by
        rw [norm_mul]
    _ ≤ (π * t) * (‖base t‖ * Real.exp (-π * ε * t)) := by
        rw [show ‖(-(π : ℂ) * (t : ℂ))‖ = π * t from by simp [abs_of_pos htpos, Real.pi_pos.le]]
        exact mul_le_mul_of_nonneg_left hnorm_integ (by nlinarith [Real.pi_pos, htpos.le])
    _ ≤ (2 / ε) * (‖base t‖ * Real.exp (-π * (ε / 2) * t)) := by
        simpa [mul_assoc, mul_left_comm, mul_comm] using
          mul_le_mul_of_nonneg_left (pi_t_exp_neg_pi_eps_le hε t) (norm_nonneg (base t))
    _ = (2 / ε) * ‖base t * (Real.exp (-π * (ε / 2) * t) : ℂ)‖ := by rw [← hnorm_base_exp]

/-- Differentiability at `u` of `z ↦ ∫ t ∈ (0,∞), base t * exp(-π z t)` when `base` is
continuous on `(0,∞)` and the integrand is integrable for every positive real parameter. -/
private lemma differentiableAt_integral_Ioi_zero_base_exp {base : ℝ → ℂ}
    (hbase_cont : ContinuousOn base (Set.Ioi (0 : ℝ)))
    (hbase_int : ∀ v : ℝ, 0 < v →
      IntegrableOn (fun t : ℝ => base t * (Real.exp (-π * v * t) : ℂ)) (Set.Ioi (0 : ℝ)))
    {u : ℂ} (hu : 0 < u.re) :
    DifferentiableAt ℂ
      (fun z : ℂ => ∫ t in Set.Ioi (0 : ℝ), base t * Complex.exp (-(π : ℂ) * z * (t : ℂ))) u := by
  set ε : ℝ := u.re / 2 with hε_def
  have hε : 0 < ε := by positivity
  have hure : u.re = 2 * ε := by simp [hε_def]; ring
  let μ : Measure ℝ := (volume : Measure ℝ).restrict (Set.Ioi (0 : ℝ))
  let F : ℂ → ℝ → ℂ := fun z t => base t * Complex.exp (-(π : ℂ) * z * (t : ℂ))
  let F' : ℂ → ℝ → ℂ := fun z t => (-(π : ℂ) * (t : ℂ)) * F z t
  have hF_meas : ∀ z : ℂ, AEStronglyMeasurable (F z) μ := fun _ => by
    have := hbase_cont.aestronglyMeasurable (μ := volume) measurableSet_Ioi
    simp only [F, μ]; fun_prop
  have hBase_ε2 :
  ...
  have hmem_Ioi : ∀ᵐ t ∂μ, t ∈ Set.Ioi (0 : ℝ) := by
    change ∀ᵐ t ∂(volume : Measure ℝ).restrict (Set.Ioi (0 : ℝ)), t ∈ Set.Ioi (0 : ℝ)
  ...
  have hF_int : Integrable (F u) μ := by
    ...
    calc ‖F u t‖ = ‖base t‖ * ‖Complex.exp (-(π : ℂ) * u * (t : ℂ))‖ := by simp [F]
      _ ≤ ‖base t‖ * Real.exp (-π * (ε / 2) * t) := mul_le_mul_of_nonneg_left
          (norm_exp_le_of_re_ge ht.le (by linarith : (ε / 2 : ℝ) ≤ u.re)) (norm_nonneg _)
      _ = ‖base t * (Real.exp (-π * (ε / 2) * t) : ℂ)‖ := by
        rw [norm_mul, Complex.norm_of_nonneg (Real.exp_nonneg _)]
  let bound : ℝ → ℝ := fun t => (2 / ε) * ‖base t * (Real.exp (-π * (ε / 2) * t) : ℂ)‖
  have hbound_int : Integrable bound μ := by simpa [bound] using hBase_ε2.norm.const_mul (2 / ε)
  have hbound : ∀ᵐ t ∂μ, ∀ z ∈ Metric.ball u ε, ‖F' z t‖ ≤ bound t := by
    filter_upwards [hmem_Ioi] with t ht z hz
    exact norm_deriv_integrand_le hε ht hure hz
  ...
  have hμ : (fun z : ℂ => ∫ t, F z t ∂μ) =
    ...
    funext fun _ => by simp [F, μ]
  exact (hμ ▸ hDeriv).differentiableAt
...
public theorem analyticOnNhd_integral_base_exp
  ...
  have hDiff : DifferentiableOn ℂ
      (fun u : ℂ => ∫ t in Set.Ioi (0 : ℝ), base t * Complex.exp (-(π : ℂ) * u * (t : ℂ)))
      rightHalfPlane := fun u hu =>
    (differentiableAt_integral_Ioi_zero_base_exp hbase_cont hbase_int
      (by simpa [rightHalfPlane] using hu)).differentiableWithinAt
