...
lemma norm_Theta2_term_resToImagAxis (n : ℤ) (t : ℝ) (ht : 0 < t) :
  ...
  rw [show (Real.pi * Complex.I * (n + (2⁻¹ : ℂ) : ℂ) ^ 2 * ((⟨(Complex.I : ℂ) * t,
        by simp [ht]⟩ : ℍ) : ℂ) : ℂ) = ((-(Real.pi * r ^ 2 * t) : ℝ) : ℂ) by
      simp [show (n + (2⁻¹ : ℂ) : ℂ) = (r : ℂ) by push_cast [r]; ring]; ring_nf; simp,
    Complex.norm_exp_ofReal]
  simp [r]

public lemma Theta3_eq_jacobiTheta (τ : ℍ) : Θ₃ τ = jacobiTheta (τ : ℂ) := rfl
    ...
    mul_le_mul_of_nonneg_right (by
      have hInv : (1 / (1 - Real.exp (-Real.pi * t))) ≤ (1 / (1 - Real.exp (-Real.pi))) :=
        one_div_le_one_div_of_le (sub_pos.2 <| Real.exp_lt_one_iff.2 (by nlinarith [Real.pi_pos]))
          (by linarith [Real.exp_le_exp.2 (show (-Real.pi * t : ℝ) ≤ -Real.pi by
...
/-- The `Θ₂` series along the imaginary axis equals twice the natural-indexed tsum of `Θ₂_term`. -/
private lemma Theta2_resToImagAxis_eq_two_tsum {t : ℝ} (htpos : 0 < t)
    (τ : ℍ) (hτ : τ = ⟨Complex.I * t, by simp [htpos]⟩) :
    Summable (fun n : ℤ ↦ Θ₂_term n τ) ∧
    Θ₂.resToImagAxis t = (2 : ℂ) * ∑' n : ℕ, Θ₂_term (n : ℤ) τ := by
  ...
  rw [show Θ₂.resToImagAxis t = Θ₂ τ by
        simp [Function.resToImagAxis, ResToImagAxis, htpos, hτ],
...
/-- Geometric bound `‖∑' a n‖ ≤ exp(-α·π·t) · (1 - exp(-π))⁻¹` whenever
`‖a n‖ ≤ exp(-α·π·t)·exp(-π)^n`. -/
private lemma tsum_geometric_exp_bound {α t : ℝ} {a : ℕ → ℂ}
    (hterm : ∀ n : ℕ, ‖a n‖ ≤ Real.exp (-α * Real.pi * t) * (Real.exp (-Real.pi)) ^ n) :
    ‖∑' n : ℕ, a n‖ ≤ Real.exp (-α * Real.pi * t) * ((1 - Real.exp (-Real.pi))⁻¹) := by
  have hgeom : HasSum (fun n : ℕ ↦ (Real.exp (-Real.pi)) ^ n) ((1 - Real.exp (-Real.pi))⁻¹) :=
    hasSum_geometric_of_lt_one (Real.exp_pos _).le <| by
      rw [Real.exp_lt_one_iff]; nlinarith [Real.pi_pos]
  exact tsum_of_norm_bounded (hgeom.mul_left (Real.exp (-α * Real.pi * t))) hterm

/-- For `t ≥ 1`, the `(n+2)`-th `Θ₂_term` along the imaginary axis decays geometrically. -/
private lemma norm_Theta2_term_shift_le {t : ℝ} (ht : 1 ≤ t) (n : ℕ) :
    ‖Θ₂_term ((n + 2 : ℕ) : ℤ) ⟨Complex.I * t, by simp [lt_of_lt_of_le zero_lt_one ht]⟩‖ ≤
      Real.exp (-(25 / 4 : ℝ) * Real.pi * t) * (Real.exp (-Real.pi)) ^ n := by
  have htpos : 0 < t := lt_of_lt_of_le zero_lt_one ht
  have hnorm : ‖Θ₂_term ((n + 2 : ℕ) : ℤ) ⟨Complex.I * t, by simp [htpos]⟩‖ =
      Real.exp (-Real.pi * (((n : ℝ) + (5 / 2 : ℝ)) ^ 2) * t) := by
    have := norm_Theta2_term_resToImagAxis (n := (n + 2 : ℕ)) (t := t) htpos
    simp at this ⊢; grind only
  have hbase : ((n : ℝ) + (5 / 2 : ℝ)) ^ 2 * t ≥ (25 / 4 : ℝ) * t + n := by
    nlinarith [ht, sq_nonneg ((n : ℝ) - (1 / 2 : ℝ)), mul_le_mul_of_nonneg_right
      ...
      (by linarith : (0:ℝ) ≤ t)]
  rw [hnorm, show (Real.exp (-Real.pi)) ^ n = Real.exp (n * (-Real.pi)) from
    (Real.exp_nat_mul (-Real.pi) n).symm, ← Real.exp_add]
  exact Real.exp_le_exp.mpr (by nlinarith [hbase, Real.pi_pos])

/-- The two-term cancellation identity: `Θ₂(it) - 2·exp(-πt/4) - 2·exp(-9πt/4) =
2 · ∑' n, Θ₂_term (n+2)`. -/
private lemma Theta2_sub_two_terms_eq {t : ℝ} (htpos : 0 < t) :
    Θ₂.resToImagAxis t - (2 : ℂ) * (Real.exp (-Real.pi * t / 4) : ℂ)
        - (2 : ℂ) * (Real.exp (-(9 / 4 : ℝ) * Real.pi * t) : ℂ)
      = (2 : ℂ) * ∑' n : ℕ,
          Θ₂_term ((n + 2 : ℕ) : ℤ) ⟨Complex.I * t, by simp [htpos]⟩ := by
  set τ : ℍ := ⟨Complex.I * t, by simp [htpos]⟩
  set f : ℕ → ℂ := fun n ↦ Θ₂_term (n : ℤ) τ
  obtain ⟨hsumZ, hΘ₂eq⟩ := Theta2_resToImagAxis_eq_two_tsum (τ := τ) htpos rfl
...
public lemma exists_bound_norm_Theta2_resToImagAxis_sub_two_terms_Ici_one :
  ...
  rw [Theta2_sub_two_terms_eq (lt_of_lt_of_le zero_lt_one ht),
    norm_mul, show ‖(2 : ℂ)‖ = (2 : ℝ) by simp,
    show (2 / (1 - Real.exp (-Real.pi))) * Real.exp (-(25 / 4 : ℝ) * Real.pi * t) =
      (2 : ℝ) * (Real.exp (-(25 / 4 : ℝ) * Real.pi * t) *
        ((1 - Real.exp (-Real.pi))⁻¹)) by ring]
  exact mul_le_mul_of_nonneg_left (tsum_geometric_exp_bound (α := 25 / 4)
    (fun n => norm_Theta2_term_shift_le ht n)) (by norm_num : (0:ℝ) ≤ 2)
...
private lemma jacobiTheta_tail_bound {τ : ℂ} {t : ℝ} (hτim : τ.im = t) (ht : 1 ≤ t) :
  ...
  rw [norm_mul, show ‖(2 : ℂ)‖ = (2 : ℝ) by simp,
    show (2 / (1 - Real.exp (-Real.pi))) * Real.exp (-(4 : ℝ) * Real.pi * t) =
      (2 : ℝ) * (Real.exp (-(4 : ℝ) * Real.pi * t) * ((1 - r)⁻¹)) by simp [r]; ring]
...
/-- Tail bound for the approximant `(2a + 2b)^4` of `Θ₂^4`, where `a := exp(-πt/4)`
and `b := exp(-9πt/4)`. After subtracting the two leading terms, the remainder
is bounded by `176·exp(-5πt)`. -/
private lemma H2_approx_tail_bound (t : ℝ) (ht : 1 ≤ t) :
    let a : ℂ := (Real.exp (-Real.pi * t / 4) : ℂ)
    let b : ℂ := (Real.exp (-(9 / 4 : ℝ) * Real.pi * t) : ℂ)
    let y : ℂ := (2 : ℂ) * a + (2 : ℂ) * b
...
public lemma exists_bound_norm_H2_resToImagAxis_sub_two_terms_Ici_one :
  ...
  have hy_tail := H2_approx_tail_bound t ht
...
private lemma theta2_norm_ge_two_exp_quarter (t : ℝ) (ht : 0 < t) :
  ...
  have hterm : ∀ n : ℤ, Θ₂_term n τ = (g n : ℂ) := fun n => by
    ...
    simpa [Θ₂_term, one_div, r, pow_two, mul_assoc, mul_left_comm, mul_comm, g, τ] using
      show Θ₂_term n τ = (Real.exp (-(Real.pi * (r ^ 2) * t)) : ℂ) by
  ...
  simpa [Function.resToImagAxis, show ‖ResToImagAxis Θ₂ t‖ = (∑' n : ℤ, g n) by
    simp [ResToImagAxis, ht, τ, show Θ₂ τ = (↑(∑' n : ℤ, g n) : ℂ) by
...
lemma H2_norm_pow_two_ge (t : ℝ) (ht0 : 0 < t) :
  ...
  have hx_ge : (16 : ℝ) * Real.exp (-Real.pi * t) ≤ ‖H₂.resToImagAxis t‖ := by
    rw [show ‖H₂.resToImagAxis t‖ = ‖Θ₂.resToImagAxis t‖ ^ (4 : ℕ) by
...
private lemma bound_w_inv_sub_one_sub (t u C0 : ℝ) (w : ℂ)
  ...
  nlinarith [show ‖w⁻¹ - (1 - ((8 * u : ℝ) : ℂ))‖ ≤
      ‖w⁻¹ - (2 - w)‖ + ‖w - (1 : ℂ) - ((8 * u : ℝ) : ℂ)‖ by
        rw [show w⁻¹ - (1 - ((8 * u : ℝ) : ℂ)) =
          (w⁻¹ - (2 - w)) - (w - (1 : ℂ) - ((8 * u : ℝ) : ℂ)) by ring]; exact norm_sub_le _ _,
    hw_tail, calc ‖w⁻¹ - (2 - w)‖
...
/-- Algebraic identity: `(16·exp(-πt) + 64·exp(-3πt))² - 256u - 2048u² = 4096·exp(-6πt)`,
where `u := exp(-2πt)`. Used inside `hw_tail_bound`. -/
private lemma main_sq_identity (t : ℝ) :
    ((16 : ℂ) * (Real.exp (-Real.pi * t) : ℂ) +
        (64 : ℂ) * (Real.exp (-(3 : ℝ) * Real.pi * t) : ℂ)) ^ (2 : ℕ)
      - (256 : ℂ) * ((Real.exp (-(2 : ℝ) * Real.pi * t) : ℝ) : ℂ)
      - (2048 : ℂ) * (((Real.exp (-(2 : ℝ) * Real.pi * t) ^ (2 : ℕ) : ℝ) : ℂ))
...
/-- Norm bound on the "main term" `16·exp(-πt) + 64·exp(-3πt)` for `1 ≤ t`. -/
private lemma main_term_norm_le (t : ℝ) (ht : 1 ≤ t) :
    ‖(16 : ℂ) * (Real.exp (-Real.pi * t) : ℂ) +
        (64 : ℂ) * (Real.exp (-(3 : ℝ) * Real.pi * t) : ℂ)‖
      ≤ 80 * Real.exp (-Real.pi * t) := by
  nlinarith [show ‖(16 : ℂ) * (Real.exp (-Real.pi * t) : ℂ) +
        (64 : ℂ) * (Real.exp (-(3 : ℝ) * Real.pi * t) : ℂ)‖ ≤
...
/-- Norm bound on `x² - 256u - 2048u²` from a bound on `x - main`, where `main` is the two-term
approximation. Combines `main_sq_identity` with the triangle inequality. -/
private lemma norm_x_sq_sub_exp_terms_le {t CH2 : ℝ} (ht : 1 ≤ t) {x : ℂ}
    (hr : ‖x - ((16 : ℂ) * (Real.exp (-Real.pi * t) : ℂ) +
        (64 : ℂ) * (Real.exp (-(3 : ℝ) * Real.pi * t) : ℂ))‖
      ≤ CH2 * Real.exp (-(5 : ℝ) * Real.pi * t)) :
    ‖x ^ (2 : ℕ) - (256 : ℂ) * ((Real.exp (-(2 : ℝ) * Real.pi * t) : ℝ) : ℂ) -
        (2048 : ℂ) * (((Real.exp (-(2 : ℝ) * Real.pi * t) ^ (2 : ℕ) : ℝ) : ℂ))‖
  ...
  have hmain_norm : ‖main‖ ≤ 80 * Real.exp (-Real.pi * t) := main_term_norm_le t ht
  have hmain_sq :
      ‖main ^ (2 : ℕ) - (256 : ℂ) * (u : ℂ) - (2048 : ℂ) * ((u ^ (2 : ℕ) : ℝ) : ℂ)‖ ≤
        (4096 : ℝ) * Real.exp (-(6 : ℝ) * Real.pi * t) := by
    rw [show main ^ (2 : ℕ) - (256 : ℂ) * (u : ℂ) - (2048 : ℂ) * ((u ^ (2 : ℕ) : ℝ) : ℂ) =
      (4096 : ℂ) * (Real.exp (-(6 : ℝ) * Real.pi * t) : ℂ) from main_sq_identity t]
    simp [abs_of_nonneg (Real.exp_pos _).le, -Complex.ofReal_exp]
  have hsq : ‖x ^ (2 : ℕ) - main ^ (2 : ℕ)‖ ≤
        (160 * Real.exp (-Real.pi * t)) * (CH2 * Real.exp (-(5 : ℝ) * Real.pi * t)) +
          (CH2 * Real.exp (-(5 : ℝ) * Real.pi * t)) ^ 2 :=
    calc ‖x ^ (2 : ℕ) - main ^ (2 : ℕ)‖ = ‖x - main‖ * ‖x + main‖ := by
          rw [show x ^ 2 - main ^ 2 = (x - main) * (x + main) by ring, norm_mul]
      _ ≤ ‖x - main‖ * ((160 * Real.exp (-Real.pi * t)) + ‖x - main‖) := by
          gcongr
          nlinarith [norm_add_le x main, norm_le_norm_add_norm_sub' x main, hmain_norm]
      _ = (160 * Real.exp (-Real.pi * t)) * ‖x - main‖ + ‖x - main‖ ^ 2 := by ring
  ...
  have htri := norm_add_le (x ^ (2 : ℕ) - main ^ (2 : ℕ))
    (main ^ (2 : ℕ) - (256 : ℂ) * (u : ℂ) - (2048 : ℂ) * ((u ^ (2 : ℕ) : ℝ) : ℂ))
  have hrw : x ^ (2 : ℕ) - (256 : ℂ) * (u : ℂ) - (2048 : ℂ) * ((u ^ (2 : ℕ) : ℝ) : ℂ) =
      (x ^ (2 : ℕ) - main ^ (2 : ℕ)) +
        (main ^ (2 : ℕ) - (256 : ℂ) * (u : ℂ) - (2048 : ℂ) * ((u ^ (2 : ℕ) : ℝ) : ℂ)) := by ring
  rw [hrw]; linarith [htri, hsq, hmain_sq]

/-- Arithmetic: `(e/256) * (sum of three exp terms) ≤ C0 * exp(-4πt)` where
`C0 = 16 + (160/256)·CH2 + CH2²/256` and `e = exp(2πt)`, for `1 ≤ t`. -/
private lemma e_div_256_mul_three_exp_terms_le (t CH2 : ℝ) (ht : 1 ≤ t) :
    (Real.exp (2 * Real.pi * t) / 256) *
        ((4096 : ℝ) * Real.exp (-(6 : ℝ) * Real.pi * t) +
          (160 * Real.exp (-Real.pi * t)) * (CH2 * Real.exp (-(5 : ℝ) * Real.pi * t)) +
          (CH2 * Real.exp (-(5 : ℝ) * Real.pi * t)) ^ 2) ≤
  ...
  nlinarith [show e * (Real.exp (-(5 : ℝ) * Real.pi * t)) ^ (2 : ℕ) =
    ...
    show (e / 256) * ((4096 : ℝ) * Real.exp (-(6 : ℝ) * Real.pi * t)) =
...
/-- Algebraic rewrite: `A·x² - 1 - 8u = A·(x² - 256u - 2048u²)`, using `e·u = 1`
with `A = e/256`. -/
private lemma A_x_sq_sub_one_sub_8u_eq {e u : ℝ} (heu : e * u = 1) (x : ℂ) :
    ((e / 256 : ℝ) : ℂ) * (x ^ (2 : ℕ)) - (1 : ℂ) - ((8 * u : ℝ) : ℂ) =
      ((e / 256 : ℝ) : ℂ) *
        (x ^ (2 : ℕ) - (256 : ℂ) * (u : ℂ) - (2048 : ℂ) * ((u ^ (2 : ℕ) : ℝ) : ℂ)) := by
  set A : ℂ := ((e / 256 : ℝ) : ℂ)
  ...
  have h2048' : A * ((2048 * (u ^ (2 : ℕ) : ℝ) : ℝ) : ℂ) = ((8 * u : ℝ) : ℂ) := by
    simp only [A]; exact_mod_cast congrArg (fun r : ℝ ↦ (r : ℂ))
      (show (e / 256) * (2048 * (u ^ (2 : ℕ) : ℝ)) = 8 * u by
        rw [show (u ^ (2 : ℕ) : ℝ) = u * u by ring]; linear_combination 8 * u * heu)
  rw [show A * (x ^ (2 : ℕ) - (256 : ℂ) * (u : ℂ) - (2048 : ℂ) * ((u ^ (2 : ℕ) : ℝ) : ℂ)) =
    A * (x ^ (2 : ℕ)) - A * ((256 * u : ℝ) : ℂ) - A * ((2048 * (u ^ (2 : ℕ) : ℝ) : ℝ) : ℂ) by
    push_cast; ring, h256', h2048']
...
private lemma hw_tail_bound (t : ℝ) (ht : 1 ≤ t) (CH2 : ℝ)
  ...
  have hr := norm_x_sq_sub_exp_terms_le (x := H₂.resToImagAxis t) ht <| by
    simpa [sub_eq_add_neg, add_assoc, add_comm] using hx_err
  calc ‖(((e / 256 : ℝ) : ℂ) * (H₂.resToImagAxis t) ^ (2 : ℕ))
          - (1 : ℂ) - ((8 * u : ℝ) : ℂ)‖
      = ‖((e / 256 : ℝ) : ℂ)‖ * ‖(H₂.resToImagAxis t) ^ (2 : ℕ) - (256 : ℂ) * (u : ℂ) -
          (2048 : ℂ) * ((u ^ (2 : ℕ) : ℝ) : ℂ)‖ := by
        rw [A_x_sq_sub_one_sub_8u_eq heu (H₂.resToImagAxis t)]; simp
    _ ≤ (e / 256) *
          ((4096 : ℝ) * Real.exp (-(6 : ℝ) * Real.pi * t) +
            (160 * Real.exp (-Real.pi * t)) * (CH2 * Real.exp (-(5 : ℝ) * Real.pi * t)) +
            (CH2 * Real.exp (-(5 : ℝ) * Real.pi * t)) ^ 2) := by
        rw [show ‖((e / 256 : ℝ) : ℂ)‖ = e / 256 by
          simpa [abs_of_nonneg he256] using (RCLike.norm_ofReal (K := ℂ) (e / 256))]
        exact mul_le_mul_of_nonneg_left hr he256
    _ ≤ (16 + (160 / 256) * CH2 + (CH2 ^ 2) / 256) * Real.exp (-(4 : ℝ) * Real.pi * t) :=
        e_div_256_mul_three_exp_terms_le t CH2 ht
...
/-- Given the tight bound `‖w - 1 - 8u‖ ≤ C0·exp(-4πt)`, derive a coarser
`‖w - 1‖ ≤ (8 + C0)·exp(-2πt)` using the triangle inequality. -/
private lemma hw_one_bound {t : ℝ} (ht : 1 ≤ t) {C0 : ℝ} (hC0 : 0 ≤ C0) {w : ℂ}
    (hw_tail : ‖w - (1 : ℂ) - ((8 * Real.exp (-(2 : ℝ) * Real.pi * t) : ℝ) : ℂ)‖
      ≤ C0 * Real.exp (-(4 : ℝ) * Real.pi * t)) :
    ‖w - (1 : ℂ)‖ ≤ (8 + C0) * Real.exp (-(2 : ℝ) * Real.pi * t) := by
  set u : ℝ := Real.exp (-(2 : ℝ) * Real.pi * t)
  ...
  refine (show ‖w - (1 : ℂ)‖ ≤ (8 + |C0|) * u from ?_).trans <| by
    simp [u, abs_of_nonneg hC0]
  linarith [show ‖w - 1‖ ≤ ‖w - (1 : ℂ) - ((8 * u : ℝ) : ℂ)‖ + ‖((8 * u : ℝ) : ℂ)‖ by
    simpa [show (w - (1 : ℂ) - ((8 * u : ℝ) : ℂ)) + ((8 * u : ℝ) : ℂ) = w - 1 by ring]
...
/-- Lower bound `1 ≤ ‖A·x²‖` where `A = exp(2πt)/256`, given `H2_norm_pow_two_ge`. -/
private lemma one_le_norm_A_mul_H2_sq {t : ℝ} (ht : 1 ≤ t) :
    (1 : ℝ) ≤ ‖((Real.exp (2 * Real.pi * t) / 256 : ℝ) : ℂ) *
        ((H₂.resToImagAxis t) ^ (2 : ℕ))‖ := by
  set e : ℝ := Real.exp (2 * Real.pi * t)
  set u : ℝ := Real.exp (-(2 : ℝ) * Real.pi * t)
  have he256 : 0 ≤ e / 256 := by positivity
  have heu : e * u = 1 := by simp [e, u, ← Real.exp_add]
  rw [show ‖((e / 256 : ℝ) : ℂ) * ((H₂.resToImagAxis t) ^ (2 : ℕ))‖ =
      (e / 256) * ‖H₂.resToImagAxis t‖ ^ (2 : ℕ) by
    rw [norm_mul, norm_pow]; congr 1; exact (RCLike.norm_ofReal (K := ℂ) (e / 256)).trans
      (abs_of_nonneg he256)]
  linarith [mul_le_mul_of_nonneg_left (show (256 : ℝ) * u ≤ ‖H₂.resToImagAxis t‖ ^ (2 : ℕ) from
      H2_norm_pow_two_ge t (lt_of_lt_of_le zero_lt_one ht)) he256,
    show (e / 256) * ((256 : ℝ) * u) = 1 by linear_combination heu]

/-- Final assembly: combine `inv_H2_sq_decomposition` with the bound `hdiff` on
`‖w⁻¹ - (1 - 8u)‖` to obtain the bound on `‖(x²)⁻¹ - A + 1/32‖`. -/
private lemma inv_H2_sq_final_bound {t : ℝ} {C0 : ℝ} (hC0 : 0 ≤ C0)
    (hdiff : ‖(((Real.exp (2 * Real.pi * t) / 256 : ℝ) : ℂ) *
        ((H₂.resToImagAxis t) ^ (2 : ℕ)))⁻¹ -
          (1 - ((8 * Real.exp (-(2 : ℝ) * Real.pi * t) : ℝ) : ℂ))‖
      ≤ ((8 + C0) ^ 2 + C0) * Real.exp (-(4 : ℝ) * Real.pi * t)) :
    ‖((H₂.resToImagAxis t) ^ (2 : ℕ))⁻¹
        - ((Real.exp (2 * Real.pi * t) / 256 : ℝ) : ℂ)
        + ((1 / 32 : ℝ) : ℂ)‖
      ≤ 256 * ((8 + C0) ^ 2 + C0) * Real.exp (-(2 : ℝ) * Real.pi * t) := by
  set e : ℝ := Real.exp (2 * Real.pi * t)
  have he256 : 0 ≤ e / 256 := by positivity
  have hA_norm : ‖((e / 256 : ℝ) : ℂ)‖ = e / 256 := by
    simpa [abs_of_nonneg he256] using (RCLike.norm_ofReal (K := ℂ) (e / 256))
  have he4 : e * Real.exp (-(4 : ℝ) * Real.pi * t) =
    Real.exp (-(2 : ℝ) * Real.pi * t) := by simp only [e, ← Real.exp_add]; ring_nf
  calc ‖((H₂.resToImagAxis t) ^ (2 : ℕ))⁻¹ - ((e / 256 : ℝ) : ℂ) + ((1 / 32 : ℝ) : ℂ)‖
      = ‖((e / 256 : ℝ) : ℂ)‖ * _ := by
        rw [inv_H2_sq_decomposition t (H₂.resToImagAxis t), norm_mul]
    _ ≤ (e / 256) * (((8 + C0) ^ 2 + C0) * Real.exp (-(4 : ℝ) * Real.pi * t)) := by
        rw [hA_norm]; exact mul_le_mul_of_nonneg_left hdiff he256
    _ ≤ 256 * ((8 + C0) ^ 2 + C0) * Real.exp (-(2 : ℝ) * Real.pi * t) := by
        nlinarith [show (0:ℝ) ≤ (8 + C0) ^ 2 + C0 by positivity,
          Real.exp_pos (-(2 : ℝ) * Real.pi * t), he4]
...
public lemma exists_bound_norm_inv_H2_sq_sub_exp_add_const_Ici_one :
  ...
  have hCH2 : 0 ≤ CH2 := nonneg_of_norm_le_mul_exp (hH2 1 le_rfl)
  have hC0 : 0 ≤ C0 := by positivity
  refine ⟨256 * ((8 + C0) ^ 2 + C0), fun t ht => ?_⟩
  set w : ℂ := ((Real.exp (2 * Real.pi * t) / 256 : ℝ) : ℂ) * ((H₂.resToImagAxis t) ^ (2 : ℕ))
  have hw_tail : ‖w - (1 : ℂ) -
      ((8 * Real.exp (-(2 : ℝ) * Real.pi * t) : ℝ) : ℂ)‖
        ≤ C0 * Real.exp (-(4 : ℝ) * Real.pi * t) := by
    simpa [w, C0] using hw_tail_bound t ht CH2 (hH2 t ht)
  have hw_norm_ge : (1 : ℝ) ≤ ‖w‖ := one_le_norm_A_mul_H2_sq ht
  exact inv_H2_sq_final_bound hC0 <| bound_w_inv_sub_one_sub t _ C0 w hw_norm_ge
    (norm_inv w ▸ inv_le_one_of_one_le₀ hw_norm_ge) hw_tail (hw_one_bound ht hC0 hw_tail)
...
/-- Cross-term bound `‖128 · (x·y) - e - 16‖ ≤ K·exp(-π t)` used in the `ψI'(it)` cancellation
estimate. Here `x ≈ x0 := 2 + 48u`, `y ≈ y0 := e/256 - 1/32`, with `u := exp(-2π t)`,
`e := exp(2π t)`. -/
private lemma psiI'_cancellation_hA {t : ℝ} (ht : 1 ≤ t) {Csum Cinv2 : ℝ}
    (hCsum : 0 ≤ Csum) (hCinv2 : 0 ≤ Cinv2)
    (x y : ℂ)
    (hx : ‖x - ((2 : ℂ) + (48 : ℂ) * ((Real.exp (-(2 : ℝ) * Real.pi * t) : ℝ) : ℂ))‖
        ≤ Csum * Real.exp (-(3 : ℝ) * Real.pi * t))
    (hy : ‖y - (((Real.exp (2 * Real.pi * t) / 256 : ℝ) : ℂ) - ((1 / 32 : ℝ) : ℂ))‖
        ≤ Cinv2 * Real.exp (-(2 : ℝ) * Real.pi * t)) :
    ‖(128 : ℂ) * (x * y) - ((Real.exp (2 * Real.pi * t) : ℝ) : ℂ) - (16 : ℂ)‖ ≤
  ...
  have hxdy : ‖(x - x0) * y0‖ ≤ (Csum + Csum / 256) * Real.exp (-Real.pi * t) := by
    have hy0_bound : ‖y0‖ ≤ (e / 256) + (1 / 32) := by
      have hexp : 0 ≤ e := (Real.exp_pos _).le
      simpa [y0, RCLike.norm_ofReal, Real.norm_eq_abs, abs_of_nonneg hexp,
  ...
  rw [show (128 : ℂ) * (x * y) - (e : ℂ) - (16 : ℂ) =
      (128 : ℂ) * ((x - x0) * y0 + x0 * (y - y0) + (x - x0) * (y - y0)) -
        (192 : ℂ) * (u : ℂ) by simp [x0, y0]; linear_combination 24 * heu]
...
/-- Cross-term bound `‖128 · (z·w) - 128‖ ≤ K·exp(-π t)` used in the `ψI'(it)` cancellation
estimate. Here `z ≈ 1` and `w ≈ 1`. -/
private lemma psiI'_cancellation_hB {t : ℝ} {z w : ℂ} {Cz Cw : ℝ}
    (hz1 : ‖z - 1‖ ≤ Cz * Real.exp (-Real.pi * t))
    (hw_bd : ‖w‖ ≤ Cw + 2)
    (hw1 : ‖w - 1‖ ≤ Cw * Real.exp (-Real.pi * t))
    (hCz : 0 ≤ Cz) :
    ‖(128 : ℂ) * (z * w) - (128 : ℂ)‖ ≤
      (128 : ℝ) * (Cz * (Cw + 2) + Cw) * Real.exp (-Real.pi * t) := by
  ...
  have hz1' : ‖z - 1‖ * ‖w‖ ≤ (Cz * Real.exp (-Real.pi * t)) * (Cw + 2) :=
...
/-- For `0 < t ≤ 1`, the modular relation `ψI = ψS|S` and the exponential decay of
`ψS.resToImagAxis` give `‖ψI'(it)‖ ≤ Cψ0`, where `Cψ0` bounds `‖ψS.resToImagAxis‖`
on `[1, ∞)`. -/
private lemma norm_psiI'_mul_I_le_of_le_one {Cψ0 : ℝ} (hCψ0 : 0 ≤ Cψ0)
    (hψS_bound : ∀ s : ℝ, 1 ≤ s → ‖ψS.resToImagAxis s‖ ≤ Cψ0 * Real.exp (-π * s))
    {t : ℝ} (ht0 : 0 < t) (ht1 : t ≤ 1) :
  ...
  nlinarith [show ‖(-(t ^ (2 : ℕ)) : ℂ)‖ = t ^ (2 : ℕ) by simp, hψS', hCψ0,
    show t ^ (2 : ℕ) ≤ 1 by simpa using pow_le_one₀ (n := 2) ht0.le ht1]
...
lemma exists_bound_norm_bAnotherBase_Ioi :
  ...
  · have hexp : ‖(Real.exp (2 * π * t) : ℂ)‖ ≤ Real.exp (2 * π) := by
    ...
    exact (by
      grind only [norm_psiI'_mul_I_le_of_le_one hCψ0 hψS_bound ht0 ht1,
        show ‖(144 : ℂ)‖ = (144 : ℝ) by norm_num] :
...
public lemma bAnotherBase_integrable_exp {u : ℝ} (hu : 0 < u) :
  ...
  have hbound :
    ...
    rw [norm_mul, show ‖(Real.exp (-π * u * t) : ℂ)‖ = Real.exp (-(π * u) * t) by
      rw [show (-π * u * t) = (-(π * u) * t) by ring, Complex.ofReal_exp,
        Complex.norm_exp_ofReal]]
