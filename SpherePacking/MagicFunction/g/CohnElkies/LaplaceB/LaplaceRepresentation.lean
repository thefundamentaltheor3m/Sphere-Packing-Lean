...
public theorem bRadial_eq_laplace_psiI_main {u : ℝ} (hu : 2 < u) :
  ...
  have hintT_shift (a : ℂ)
    ...
    have hConst : IntegrableOn (fun t : ℝ =>
      ...
      simpa [mul_assoc] using
        ((hintI.mono_set (Set.Ioi_subset_Ioi (by norm_num : (0 : ℝ) ≤ 1))).neg.mul_const
          (bContourWeight u a))
  ...
  have htendstoT :
    ...
    have htdec :
      ...
      simpa [mul_assoc] using tendsto_const_nhds.mul
        (Real.tendsto_exp_neg_atTop_nhds_zero.comp
          (by simpa [mul_assoc, mul_comm, mul_left_comm] using tendsto_id.const_mul_atTop hc))
    intro ε hε
    ...
    rw [show ‖bContourIntegrandT u z‖ = ‖ψT' z‖ * ‖bContourWeight u z‖ from by
        ...
        ψT'_eq_ψI'_add_one z hzpos, show ψI' (z + (1 : ℂ)) =
          ψI ⟨z + (1 : ℂ), hzI⟩ by simp [ψI', hzpos],
        show ‖bContourWeight u z‖ = Real.exp (-π * u * z.im) from by
          simp [bContourWeight, Complex.norm_exp]]
    refine lt_of_le_of_lt ?_ (hMε z.im ((le_max_right _ _).trans hzM))
    refine (mul_le_mul_of_nonneg_right hψI (Real.exp_pos _).le).trans (le_of_eq ?_)
  ...
  have hRectLeft :
    ...
    have hcont' : ContinuousOn f (Set.uIcc (0 : ℝ) 1 ×ℂ Set.Ici (1 : ℝ)) :=
      (continuousOn_bContourIntegrandT (u := u)).comp
        (continuousOn_id.sub continuousOn_const) fun z hz => by
          simpa [sub_eq_add_neg] using hStrip0 (by
    ...
    have hdiff' :
      ...
      have hzpos' : 0 < (z - 1).im := by
        simpa [sub_eq_add_neg] using lt_trans (by norm_num : (0 : ℝ) < 1) ((Set.mem_Ioi).1 hz.1.2)
      simpa [f] using ((differentiableOn_bContourIntegrandT (u := u) (z - 1) hzpos').differentiableAt
      ...
      intervalIntegral.integral_congr fun t ht => by
        have hz_g : MagicFunction.Parametrisations.z₄' t =
          ...
          linear_combination (by
            ...
            MagicFunction.Parametrisations.z₄' t = (1 : ℂ) - (t : ℂ) + (I : ℂ))
  ...
  have hCenter_split : (∫ t in Set.Ioi (1 : ℝ), bContourIntegrandS u (I * (t : ℂ))) =
    ...
    simpa [MeasureTheory.integral_neg, sub_eq_add_neg, add_assoc, add_left_comm, add_comm] using
      MeasureTheory.integral_sub (μ := volume.restrict (Set.Ioi (1 : ℝ)))
        (hintI.mono_set (Set.Ioi_subset_Ioi (by norm_num : (0 : ℝ) ≤ 1))).neg hintT_center
  have hVI_split : VI =
