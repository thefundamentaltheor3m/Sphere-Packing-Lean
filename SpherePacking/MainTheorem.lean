...
`scaledMagic`, and combines the Cohn-Elkies linear programming bound with the density of the `E8`
lattice packing to obtain the dimension-8 optimal packing density.

## Main statements

* `SpherePacking.MainTheorem`: `SpherePackingConstant 8 = E8Packing.density`
* `SpherePacking.SpherePackingConstant_le_E8Packing_density`
* `g_real` / `g_real_fourier` (blueprint `thm:g1` / `thm:g`): `g` and `𝓕 g` are real-valued.
-/
