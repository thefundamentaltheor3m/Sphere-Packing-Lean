...
public lemma exists_bound_pow_mul_exp_neg_mul (k : ℕ) {b : ℝ} (hb : 0 < b) :
  ...
  rcases (eventually_atTop.1 <|
    ht.eventually (Iio_mem_nhds zero_lt_one)) with ⟨A, hA⟩
  let A' : ℝ := max A 0
  have hcont : Continuous f := by fun_prop
  rcases (isCompact_Icc : IsCompact (Set.Icc (0 : ℝ) A')).exists_isMaxOn
      (Set.nonempty_Icc.2 (le_max_right A 0)) hcont.continuousOn with ⟨x0, hx0, hxmax⟩
  refine ⟨max (f x0) 1, fun x hx => ?_⟩
  by_cases hxA : x ≤ A'
  · exact (hxmax ⟨hx, hxA⟩).trans (le_max_left _ _)
  · exact (le_of_lt (hA x (le_trans (le_max_left _ _) (le_of_not_ge hxA)))).trans
      (le_max_right _ _)
