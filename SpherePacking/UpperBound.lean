...
Develops the integral representations of Viazovska's magic function `g` and its Fourier
transform, and establishes the Cohn-Elkies sign conditions
`g_nonpos_of_norm_sq_ge_two` and `fourier_g_nonneg`.

The scaled function `scaledMagic`, the real-valuedness lemmas `g_real` / `g_real_fourier`
(blueprint `thm:g1` / `thm:g`), and the main theorem itself live in
`SpherePacking.MainTheorem`. The E₈ lattice and packing material live in `SpherePacking.E8.Basic`
and `SpherePacking.E8.Packing`.
-/
...
/-- Continuity of the Laplace integrand `bLaplaceIntegrand u` on `(0, ∞)`. -/
public lemma continuousOn_bLaplaceIntegrand (u : ℝ) :
    ContinuousOn (fun t : ℝ => bLaplaceIntegrand u t) (Set.Ioi (0 : ℝ)) := fun t ht0 => by
  have hψIcomp :
      ContinuousAt (fun s : ℝ => ψI (UpperHalfPlane.ofComplex ((Complex.I : ℂ) * (s : ℂ)))) t :=
...
/-- Integrability of `bLaplaceIntegrand u` on `(0, 1]` for `u ≥ 0`. Uses the `S`-slash-action of
`ψS` to reduce the bound near `0` to the exponential decay of `ψS` at infinity. -/
private lemma bLaplaceIntegrand_integrable_Ioc_zero_one {u : ℝ} (hu0 : 0 ≤ u) :
  ...
  refine IntegrableOn.of_bound measure_Ioc_lt_top
    (((continuousOn_bLaplaceIntegrand u).mono fun t ht => ht.1).aestronglyMeasurable
      measurableSet_Ioc) Cψ0 ?_
...
/-- Integrability of `bLaplaceIntegrand u` on `(A, ∞)` whenever `AI ≤ A` and `ψI` enjoys the
exponential growth bound `‖ψI z‖ ≤ CI * exp(2π z.im)` for `z.im ≥ AI`. -/
private lemma bLaplaceIntegrand_integrable_Ioi_of_ψI_bound {u : ℝ} (hu : 2 < u)
    {CI AI A : ℝ} (hCI : 0 < CI)
    (hI : ∀ z : ℍ, AI ≤ z.im → ‖ψI z‖ ≤ CI * Real.exp (2 * π * z.im))
    (hA0 : 0 < A) (hAAI : AI ≤ A) :
  ...
  have hmeas : AEStronglyMeasurable (fun t : ℝ => bLaplaceIntegrand u t)
    ...
    ((continuousOn_bLaplaceIntegrand u).mono fun t ht =>
      lt_trans hA0 ht).aestronglyMeasurable measurableSet_Ioi
  have hdom :
            ...
            mul_le_mul_of_nonneg_right (by
              simpa [show ψI' ((Complex.I : ℂ) * (t : ℂ)) =
                ...
                hI (⟨(Complex.I : ℂ) * (t : ℂ), htIm⟩ : ℍ)
                  (by simpa [UpperHalfPlane.im] using hAAI.trans ht.le))
              (Real.exp_pos _).le
...
public lemma bLaplaceIntegral_convergent {u : ℝ} (hu : 2 < u) :
  ...
  obtain ⟨CI, AI, hCI, hI⟩ := exists_ψI_bound_exp
  set A : ℝ := max AI 1
  have hA1 : (1 : ℝ) ≤ A := le_max_right _ _
  have hint_mid : IntegrableOn (fun t : ℝ => bLaplaceIntegrand u t) (Set.Ioc (1 : ℝ) A) :=
    (((continuousOn_bLaplaceIntegrand u).mono fun t ht =>
      lt_of_lt_of_le zero_lt_one ht.1).integrableOn_Icc).mono_set Set.Ioc_subset_Icc_self
  rw [show Set.Ioi (0 : ℝ) = Set.Ioc (0 : ℝ) 1 ∪ Set.Ioi (1 : ℝ) from by norm_num,
    show Set.Ioi (1 : ℝ) = Set.Ioc (1 : ℝ) A ∪ Set.Ioi A from (Set.Ioc_union_Ioi_eq_Ioi hA1).symm]
  exact (bLaplaceIntegrand_integrable_Ioc_zero_one (by linarith)).union (hint_mid.union
    (bLaplaceIntegrand_integrable_Ioi_of_ψI_bound (u := u) hu hCI hI
      (lt_of_lt_of_le zero_lt_one hA1) (le_max_left _ _)))
...
/-- The closed strip `[0,1] × [1, ∞)` lies in the open upper half-plane. -/
private lemma laplacePsiI_strip0_subset :
    (Set.uIcc (0 : ℝ) 1 ×ℂ Set.Ici (1 : ℝ)) ⊆ {z : ℂ | 0 < z.im} :=
  fun _ hz => lt_of_lt_of_le zero_lt_one (by simpa using hz.2)

/-- Integrability of `bContourIntegrandI` on `(0, ∞)` along the imaginary axis. -/
private lemma laplacePsiI_intI {u : ℝ} (hu : 2 < u) :
    IntegrableOn (fun t : ℝ => bContourIntegrandI u (I * (t : ℂ))) (Set.Ioi (0 : ℝ)) := by
...
/-- Integrability of `bContourIntegrandT` on `(1, ∞)` along the imaginary axis. -/
private lemma laplacePsiI_intT_center {u : ℝ} (hu : 2 < u) :
    IntegrableOn (fun t : ℝ => bContourIntegrandT u (I * (t : ℂ))) (Set.Ioi (1 : ℝ)) := by
...
/-- Integrability of `bContourIntegrandT` along the shifted ray `a + I·t` on `(1, ∞)`. -/
private lemma laplacePsiI_intT_shift {u : ℝ} (hu : 2 < u) (a : ℂ)
  ...
  refine (show IntegrableOn (fun t : ℝ =>
    ...
    simpa [mul_assoc] using
      (((laplacePsiI_intI hu).mono_set (Set.Ioi_subset_Ioi zero_le_one)).neg.mul_const
...
/-- Decay of `‖bContourIntegrandT u z‖` to zero as `Im z → ∞`. -/
private lemma laplacePsiI_tendstoT {u : ℝ} (hu : 2 < u) :
...
/-- The left rectangle contour integral vanishes: the boundary integral of `bContourIntegrandT`
on the rectangle with left side `x = -1`. -/
private lemma laplacePsiI_rectLeft {u : ℝ} (hu : 2 < u) :
    (∫ (x : ℝ) in (0 : ℝ)..1,
    ...
    integral_boundary_open_rect_eq_zero_of_differentiable_on_off_countable_of_integrable_on
      ...
      ((continuousOn_bContourIntegrandT (u := u)).comp
        (continuousOn_id.sub continuousOn_const) fun z hz => by
          simpa [sub_eq_add_neg] using laplacePsiI_strip0_subset
            (by simpa [Set.uIcc_of_le zero_le_one] using hz))
      ...
      (by simpa [f, sub_eq_add_neg, add_assoc, add_left_comm, add_comm, mul_assoc, mul_comm,
          mul_left_comm] using laplacePsiI_intT_shift hu (-1 : ℂ) fun t ht0 ↦
            by simpa [add_assoc] using ψT'_neg_one_add_I_mul (t := t) ht0)
      (by simpa [f, sub_eq_add_neg, add_assoc, add_left_comm, add_comm, mul_assoc, mul_comm,
          mul_left_comm] using laplacePsiI_intT_center hu)
      (fun ε hε => let ⟨M, hM⟩ := laplacePsiI_tendstoT hu ε hε
        ⟨M, fun z hz => by simpa [f] using hM (z - 1) (by simpa [sub_eq_add_neg] using hz)⟩)

/-- The right rectangle contour integral vanishes: the boundary integral of `bContourIntegrandT`
on the rectangle with right side `x = 1`. -/
private lemma laplacePsiI_rectRight {u : ℝ} (hu : 2 < u) :
  ...
  simpa [min_eq_right zero_le_one, max_eq_left zero_le_one, mul_assoc, mul_comm, mul_left_comm,
    ...
    integral_boundary_open_rect_eq_zero_of_differentiable_on_off_countable_of_integrable_on
      ...
      (by simpa [Set.uIcc_comm] using
        (continuousOn_bContourIntegrandT (u := u)).mono laplacePsiI_strip0_subset)
      (s := (∅ : Set ℂ)) (by simp)
      ...
      (by simpa [mul_comm, mul_left_comm, mul_assoc, add_assoc, add_left_comm, add_comm] using
        laplacePsiI_intT_shift hu (1 : ℂ) fun t ht0 ↦
...
private lemma laplacePsiI_ITS {u : ℝ} (z : ℂ) (hz : 0 < z.im) :
...
/-- Splitting of `∫ bContourIntegrandS` on `(1, ∞)` using `ψI = ψT + ψS`. -/
private lemma laplacePsiI_center_split {u : ℝ} (hu : 2 < u) :
  ...
  rw [show (∫ t in Set.Ioi (1 : ℝ), bContourIntegrandS u ((Complex.I : ℂ) * (t : ℂ))) =
    ...
    MeasureTheory.setIntegral_congr_fun measurableSet_Ioi fun t ht => by
      ...
      with_reducible exact eq_sub_iff_add_eq'.mpr (laplacePsiI_ITS (u := u) (I * ↑t) hz)]
  simpa [MeasureTheory.integral_neg, sub_eq_add_neg, add_assoc, add_left_comm, add_comm] using
    MeasureTheory.integral_sub (μ := volume.restrict (Set.Ioi (1 : ℝ)))
      ((laplacePsiI_intI hu).mono_set (Set.Ioi_subset_Ioi zero_le_one)).neg
      (laplacePsiI_intT_center hu)

open MagicFunction.b.RealIntegrals in
/-- Rewriting `J₂'` as an integral of `bContourIntegrandT` on `[0, 1]`. -/
private lemma laplacePsiI_J2_top {u : ℝ} : J₂' u =
    ∫ (x : ℝ) in (0 : ℝ)..1,
      bContourIntegrandT u ((x : ℂ) + (1 : ℂ) * Complex.I - 1) := by
  refine intervalIntegral.integral_congr fun x hx ↦ ?_
  have hmem : x ∈ Set.Icc (0 : ℝ) 1 := by simpa [Set.uIcc_of_le zero_le_one] using hx
  simp [bContourIntegrandT, bContourWeight, sub_eq_add_neg, mul_assoc,
    show z₂' x = (x : ℂ) + (1 : ℂ) * Complex.I - 1 by
      have h := z₂'_eq_of_mem (t := x) hmem; push_cast at h; linear_combination h]

open MagicFunction.b.RealIntegrals in
/-- Rewriting `J₄'` as an integral of `bContourIntegrandT` on `[1, 0]`. -/
private lemma laplacePsiI_J4_top {u : ℝ} : J₄' u =
    ∫ (x : ℝ) in (1 : ℝ)..0,
  ...
  rw [intervalIntegral.integral_congr fun t ht => show _ = (-1 : ℂ) * g (1 - t) by
      have hmem : t ∈ Set.Icc (0 : ℝ) 1 := by simpa [Set.uIcc_of_le zero_le_one] using ht
      simp [g, bContourIntegrandT, bContourWeight, sub_eq_add_neg, mul_assoc,
        show z₄' t = ((1 - t : ℝ) : ℂ) + (1 : ℂ) * Complex.I by
          have h := z₄'_eq_of_mem (t := t) hmem; push_cast at h ⊢; linear_combination h],
...
/-- Auxiliary identity rewriting a vertical-segment integral using `bContourIntegrandT`. -/
private lemma laplacePsiI_J_vert_aux {u : ℝ} (a : ℂ) (zp : ℝ → ℂ)
  ...
  rw [intervalIntegral.integral_congr fun t ht =>
      show _ = (I : ℂ) * bContourIntegrandT u (a + I * (t : ℂ)) by
        have hmem : t ∈ Set.Icc (0 : ℝ) 1 := by simpa [Set.uIcc_of_le zero_le_one] using ht
        simp [bContourIntegrandT, bContourWeight, hzp hmem, mul_assoc]]
  rw [show (∫ x in (0 : ℝ)..1, (I : ℂ) * bContourIntegrandT u (a + I * (x : ℂ))) =
      (I : ℂ) * ∫ x in (0 : ℝ)..1, bContourIntegrandT u (a + I * (x : ℂ)) from
    intervalIntegral.integral_const_mul _ _,
    intervalIntegral.integral_of_le zero_le_one]

/-- Shifted-integrand identity: along the contour `a + I·t`, `bContourIntegrandT` factors as
`bContourIntegrandI · (-bContourWeight a)`. -/
private lemma laplacePsiI_shift_point {u : ℝ} (a : ℂ)
    (hψa : ∀ t : ℝ, 0 < t → ψT' (a + I * (t : ℂ)) = ψI' (I * (t : ℂ)))
...
open MagicFunction.b.RealIntegrals in
/-- Reassembled sum identity: the six `J`-integrals add up to
`I · ((2 - bContourWeight 1 - bContourWeight (-1)) · VI)` where `VI` is the central
imaginary-axis integral of `bContourIntegrandI`. -/
private lemma laplacePsiI_hsum {u : ℝ} (hu : 2 < u) :
    J₁' u + J₂' u + J₃' u + J₄' u + J₅' u + J₆' u =
      (Complex.I : ℂ) *
        (((2 : ℂ) - bContourWeight u (1 : ℂ) - bContourWeight u (-1 : ℂ)) *
          (∫ t in Set.Ioi (0 : ℝ), bContourIntegrandI u ((Complex.I : ℂ) * (t : ℂ)))) := by
  set VI : ℂ := ∫ t in Set.Ioi (0 : ℝ), bContourIntegrandI u ((Complex.I : ℂ) * (t : ℂ))
  have hintI := laplacePsiI_intI hu
  have hRectLeft := laplacePsiI_rectLeft hu
  have hRectRight := laplacePsiI_rectRight hu
  have hCenter_split := laplacePsiI_center_split hu
  have hJ2_top := laplacePsiI_J2_top (u := u)
  have hJ4_top := laplacePsiI_J4_top (u := u)
  ...
  have hJ1_set : J₁' u =
    ...
    laplacePsiI_J_vert_aux (u := u) (-1 : ℂ) z₁' fun ht => by simpa using z₁'_eq_of_mem ht
  have hJ3_set : J₃' u =
      (I : ℂ) * (∫ t in Set.Ioc (0 : ℝ) 1, bContourIntegrandT u ((1 : ℂ) + I * (t : ℂ))) :=
    laplacePsiI_J_vert_aux (u := u) (1 : ℂ) z₃' fun ht => by simpa using z₃'_eq_of_mem ht
  have hJ5_set : J₅' u =
  ...
  have hLeft_full := hfull (-1 : ℂ)
    (laplacePsiI_shift_point (u := u) (-1 : ℂ) ψT'_neg_one_add_I_mul)
  have hRight_full := hfull (1 : ℂ)
    (laplacePsiI_shift_point (u := u) (1 : ℂ) ψT'_one_add_I_mul)
...
public theorem bRadial_eq_laplace_psiI_main {u : ℝ} (hu : 2 < u) :
  ...
  simpa [bContourWeight, sub_eq_add_neg, add_left_comm, add_comm, mul_assoc, mul_left_comm,
    mul_comm] using laplacePsiI_hsum hu
  ...
  (show J₄'C = fun u : ℂ => (-1 : ℂ) * ∫ t in (0 : ℝ)..1,
    ...
    (differentiable_const (-1 : ℂ)).mul (integral_ψ_exp_differentiable (Cz := 3)
      ...
      continuous_z₄' hMψ (fun t ht => by
        have htic := mem_Icc_of_Ioc (mem_Ioc_of_mem_uIoc ht)
        exact (show z₄' t = ((1 : ℂ) + (-(t : ℂ))) + (Complex.I : ℂ) by
            simp [z₄'_eq_of_mem (t := t) htic, sub_eq_add_neg, add_comm]) ▸
          norm_add_I_le_three _ htic
...
lemma J₅'C_differentiable : Differentiable ℂ J₅'C :=
  ...
  (show J₅'C = fun u : ℂ => (-2 * Complex.I : ℂ) * ∫ t in (0 : ℝ)..1,
    ...
    funext fun u => by
      simp only [J₅'C, mul_assoc, mul_comm]
...
/-- Integrability of the `b` correction integrand `(144 + exp(2πt)) * exp(-πut)`. -/
private lemma bCorr_integrableOn {u : ℝ} (hu : 2 < u) :
    IntegrableOn
      (fun t : ℝ => ((144 : ℂ) + ((Real.exp (2 * π * t) : ℝ) : ℂ)) * Real.exp (-π * u * t))
      (Set.Ioi (0 : ℝ)) :=
  (((integrableOn_exp_neg_pi_mul_Ioi_complex (lt_trans (by norm_num) hu)).const_mul
      (144 : ℂ)).add (integrableOn_exp_two_pi_mul_exp_neg_pi_mul_Ioi_complex hu)).congr <|
    MeasureTheory.ae_restrict_of_forall_mem measurableSet_Ioi fun t _ => by
      simp [-Complex.ofReal_exp, add_mul, mul_assoc]

/-- Evaluation of the `b` correction integral: it equals `144/(πu) + 1/(π(u-2))`. -/
private lemma bCorr_integral_eval {u : ℝ} (hu : 2 < u) :
    (∫ t in Set.Ioi (0 : ℝ),
  ...
  rw [show (fun t : ℝ => ((144 : ℂ) + ((Real.exp (2 * π * t) : ℝ) : ℂ)) * Real.exp (-π * u * t)) =
    ...
    MeasureTheory.integral_add ((integrableOn_exp_neg_pi_mul_Ioi_complex hu0).const_mul (144 : ℂ))
...
/-- Decomposition of the `b` Laplace integral into the "another integral" plus the correction
integral. -/
private lemma bLaplaceIntegral_decomp {u : ℝ} (hu : 2 < u) :
  ...
  have hCorrInt := bCorr_integrableOn hu
  ...
  rw [MeasureTheory.setIntegral_congr_fun (μ := (volume : Measure ℝ)) (s := Set.Ioi (0 : ℝ))
    measurableSet_Ioi (g := fun t : ℝ => bAnotherIntegrand u t +
      ((144 : ℂ) + ((Real.exp (2 * π * t) : ℝ) : ℂ)) * Real.exp (-π * u * t))
    (fun t _ => by simp [hpoint t])]
...
lemma bRadial_eq_another_integral_of_gt2 {u : ℝ} (hu : 2 < u) : b' u =
  ...
  rw [show b' u = (-4 * (Complex.I : ℂ)) * (Real.sin (π * u / 2)) ^ (2 : ℕ) *
    ...
    bLaplaceIntegral_decomp hu, bCorr_integral_eval hu]
  ring_nf
...
on `(0, ∞)` for `u > 2`. -/
private lemma aCorr_integrableOn {u : ℝ} (hu : 2 < u) (c36 c8640 c18144 : ℂ) :
    IntegrableOn (fun t : ℝ =>
        (c36 * Real.exp (2 * π * t) - c8640 * t + c18144) * Real.exp (-π * u * t))
      (Set.Ioi (0 : ℝ)) :=
  (((((integrableOn_exp_two_pi_mul_exp_neg_pi_mul_Ioi_complex hu).const_mul c36).sub
      ((integrableOn_mul_exp_neg_pi_mul_Ioi_complex (lt_trans (by norm_num) hu)).const_mul
        c8640)).add
        ((integrableOn_exp_neg_pi_mul_Ioi_complex (lt_trans (by norm_num) hu)).const_mul
          c18144))).congr <|
    MeasureTheory.ae_restrict_of_forall_mem measurableSet_Ioi fun t _ ↦ by dsimp; ring

/-- The Laplace integral `∫ aLaplaceIntegrand u t` equals the sum of `aAnotherIntegrand u t` and
the correction integral. -/
private lemma aLaplaceIntegral_decomp {u : ℝ} (hu0 : 0 < u) (hu : 2 < u)
    {c36 c8640 c18144 : ℂ} (hc36 : c36 = ((36 / (π ^ (2 : ℕ)) : ℝ) : ℂ))
    (hc8640 : c8640 = ((8640 / π : ℝ) : ℂ))
    (hc18144 : c18144 = ((18144 / (π ^ (2 : ℕ)) : ℝ) : ℂ))
    (corr : ℝ → ℂ)
    (hcorr : corr = fun t : ℝ =>
  ...
  rw [MeasureTheory.setIntegral_congr_fun measurableSet_Ioi
    (g := fun t : ℝ => aAnotherIntegrand u t + corr t) fun t _ ↦ by
    subst hc36 hc8640 hc18144 hcorr
    simp [-Complex.ofReal_exp, aLaplaceIntegrand, aAnotherIntegrand,
      sub_eq_add_neg, add_left_comm, add_comm, mul_assoc, mul_left_comm, mul_comm]
    ring]
  exact integral_add (by simpa [MeasureTheory.IntegrableOn] using
    aAnotherIntegrand_integrable_of_pos hu0)
    (by simpa [MeasureTheory.IntegrableOn, hcorr] using aCorr_integrableOn hu c36 c8640 c18144)
...
lemma aRadial_eq_another_integral_of_gt2 {u : ℝ} (hu : 2 < u) :
  ...
  let corr : ℝ → ℂ :=
    fun t : ℝ => (c36 * Real.exp (2 * π * t) - c8640 * t + c18144) * Real.exp (-π * u * t)
  simpa [aAnotherIntegral, aLaplaceIntegral_decomp hu0 hu hc36 hc8640 hc18144 corr rfl,
    show (∫ t in Set.Ioi (0 : ℝ), corr t) =
      (36 : ℂ) / (π ^ (3 : ℕ) * (u - 2)) -
        (8640 : ℂ) / (π ^ (3 : ℕ) * u ^ (2 : ℕ)) + (18144 : ℂ) / (π ^ (3 : ℕ) * u) from
    corrIntegral_eval hc36 hc8640 hc18144 rfl
      (integral_exp_neg_pi_mul_Ioi_complex hu0)
      (integral_mul_exp_neg_pi_mul_Ioi_complex hu0)
      (integral_exp_two_pi_mul_exp_neg_pi_mul_Ioi_complex hu)
...
lemma integral_B_mul_exp_decomp {u : ℝ} (hu : 0 < u) :
  ...
  simp only [f1, f2, f3, f4, MeasureTheory.integral_neg, μ, sub_eq_add_neg, add_assoc,
...
public lemma ψI'_re_mul_I (t : ℝ) (ht : 0 < t) :
  ...
  have hψS' : ψS.resToImagAxis s = ((-(s ^ (2 : ℕ)) : ℝ) : ℂ) * ψI.resToImagAxis t := by
    simpa [show (1 / s) = t by simp [s], zpow_ofNat, pow_two,
      ...
      ResToImagAxis.SlashActionS (F := ψI) (k := (-2 : ℤ)) (t := s) (one_div_pos.2 ht)
...
public lemma A_eq_neg_mul_FG_div_Delta (t : ℝ) (ht : 0 < t) :
  ...
  field_simp [ne_of_gt (Delta_imag_axis_pos.2 s hs)]; ring
...
/-- Pointwise decomposition of the kernel `A t`. -/
private lemma A_eq_decomposition {t : ℝ} (ht : t ∈ Set.Ioi (0 : ℝ)) :
    (A t : ℂ) = (-(t ^ (2 : ℕ)) : ℂ) * φ₀'' ((Complex.I : ℂ) / (t : ℂ)) +
      (-(36 / (π ^ (2 : ℕ)) : ℝ) : ℂ) * ψI' ((Complex.I : ℂ) * (t : ℂ)) := by
  apply Complex.ext <;> simp [A, sub_eq_add_neg, φ₀''_imag_axis_div_im t ht,
    ψI'_imag_axis_im t ht]

/-- The kernel `A`-integral splits into the (negative of the) `a`-Laplace integrand and a
multiple of the `b`-Laplace integrand. -/
private lemma A_integral_split {u : ℝ} (hu : 2 < u) :
    (∫ t in Set.Ioi (0 : ℝ), (A t : ℂ) * Real.exp (-π * u * t)) =
      -(∫ t in Set.Ioi (0 : ℝ),
          ((t ^ (2 : ℕ) : ℝ) : ℂ) * φ₀'' ((Complex.I : ℂ) / (t : ℂ)) * Real.exp (-π * u * t)) +
        (-(36 / (π ^ (2 : ℕ)) : ℝ) : ℂ) *
          ∫ t in Set.Ioi (0 : ℝ),
            ψI' ((Complex.I : ℂ) * (t : ℂ)) * Real.exp (-π * u * t) := by
  ...
  have hset : (∫ t in Set.Ioi (0 : ℝ), (A t : ℂ) * Real.exp (-π * u * t)) =
    ...
    MeasureTheory.setIntegral_congr_fun (μ := (volume : Measure ℝ)) (s := Set.Ioi (0 : ℝ))
      measurableSet_Ioi (fun t ht => by simp [A_eq_decomposition ht])
  rw [hset]
...
/-- Algebraic identity combining the `a` and `b` prefactors into the `gRadial` constants. -/
private lemma gRadial_coef_combine {u : ℝ} (IA IB : ℂ) :
    ((↑π * Complex.I) / 8640 : ℂ) *
...
public theorem gRadial_eq_integral_A {u : ℝ} (hu : 2 < u) : gRadial u =
  ...
  have hA_integral : (∫ t in Set.Ioi (0 : ℝ), (A t : ℂ) * Real.exp (-π * u * t)) =
      -IA + (-(36 / (π ^ (2 : ℕ)) : ℝ) : ℂ) * IB := A_integral_split (u := u) hu
  have hmain := gRadial_coef_combine (u := u) IA IB
  simp_all
